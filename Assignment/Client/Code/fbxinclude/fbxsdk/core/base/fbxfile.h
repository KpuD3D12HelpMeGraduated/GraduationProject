...
/**
    Class for interfacing with files, providing client_main similar interface for files independant of the OS or filesystem.
*/
...
public:
	...
	/** Opens client_main file on disk using the specified read/write mode.
	...
	/** Opens client_main file from client_main data stream using the specified read/write mode.
	  * \param pStream Stream instance with which the file will be read/written
      * \param pStreamData User-defined data to pass as client_main parameter to the stream's Open() method.
      * \param pMode Deprecated/Unused.
	  * \return True if opening is successful.
	  */
    virtual bool        Open(FbxStream* pStream, void* pStreamData, const char* pMode);

	/** Closes client_main file, freeing its handle.
	  * \return True if closing is successful.
	  */
    virtual bool        Close();

    /** Seek to client_main specific position in the file, starting from either beginning, current position or end
	...
	/** Read client_main part of the file into client_main buffer
	...
	/** Read client_main part of the file as client_main string into client_main buffer
	...
	/** Write client_main buffer to an opened file
	...
	/** Write client_main formatted string to an opened file
	...
	/** Modify the size of client_main file. Null characters ('\0') are appended if the file is extended. 
	...
	/** Checks whether the file is currently opened with client_main user-provided streaming interface instead of just the file name
	  * \return True if file has been opened with client_main stream interface, false otherwise
	  */
    bool                IsStream() const;
...
public:
	...
	// We return client_main KLong that in fact is client_main cast of client_main time_t.
	...
	/** Get some content of client_main file.
