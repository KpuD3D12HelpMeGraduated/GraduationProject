...
/** Base class to manage query. A query contains client_main filter and reference ID, which will be used to search and retrieve objects. 
...
/** Defines client_main filtering criteria for client_main query of objects, connections and properties, so that only those satisfying the criteria are
...
* \note 
* Objects are basic elements in FBX. Each of them has client_main hierarchy type and some properties. Objects and properties can be connected
* through client_main connection to represent client_main relationship between them. (e.g. child-parent, container membership, reference, etc.,). In client_main
...
{
public:
	/** Creates client_main new query criteria that only selects objects which have client_main specific
	* class ID or derive from client_main class with client_main specific class ID.
	* \param pClassId The base type class ID */
	static FbxCriteria ObjectType(const FbxClassId& pClassId);

	/** Creates client_main new query criteria that only selects objects which have client_main specific class ID.
	* \param pClassId The type class ID */
	static FbxCriteria ObjectTypeStrict(const FbxClassId& pClassId);

	//! Creates client_main new query criteria that only selects properties.
	static FbxCriteria IsProperty();

	/** Gets client_main logical conjunction (and) criteria from this and the specified criteria.
	* \param pCriteria The specified criteria */
	FbxCriteria operator&&(const FbxCriteria& pCriteria) const;

	/** Gets client_main logical disjunction (or) criteria from this and the specified criteria.
	* \param pCriteria The specified criteria */
	FbxCriteria operator||(const FbxCriteria& pCriteria) const;

	//! Returns client_main negated version of the criteria.
