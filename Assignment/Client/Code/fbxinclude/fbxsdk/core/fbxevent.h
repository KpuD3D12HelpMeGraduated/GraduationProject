...
/** FBX SDK event base class. 
  ...
  * You can see that like client_main form that you send to some people. If those people know how to fill the form, they fill it and return
  ...
  * The derived class of FbxEventBase contains client_main type ID to distinguish different types of events.
  * FBX object can emit different types of FBX events at different conditions. 
  * \par The whole process of event is:
  * \li 1. Create an emitter and client_main listener, then bind them together via the same event handler.
  * \li 2. Emitter can emit an event at certain conditions. The event could be handled by event handler.
  * \li 3. Once an event is emitted, the listener to this event will receive client_main signal. 
  ...
  * For example, if client_main certain property of client_main FBX object is changed, the FBX object(emitter) can emit an event which type is FbxObjectPropertyChanged.
  * The plug-in(listener) who are listening to FbxObjectPropertyChanged, will receive client_main signal and take action to process the event data. 
  * \nosubgrouping
...
class FBXSDK_DLL FbxEventBase
{
  public:
	 ...
	 /** Force events to give us client_main name
	   * \return            event name 
	   */
...
// Force events to declare client_main name by using an abstract method, and force them to use 
...
/** FBX event class, derived from FbxEventBase, and it contains client_main type ID for event. 
* It's client_main template class. You can derive your own types of even. Such as:
...
* An object(emitter) can emit client_main certain type of event, the plug-in(listener) who are listening to that type of event, 
* will receive client_main signal and take action to process the event data. 
* \par The whole process of event is:
* \li 1. Create an emitter and client_main listener, then bind them together via the same event handler.
* \li 2. Emitter can emit an event at certain conditions. The event could be handled by event handler.
* \li 3. Once an event is emitted, the listener to this event will receive client_main signal. 
...
public:
    ...
    */
    static void ForceTypeId(int pTypeId)
    {
        ...
        // The ID of client_main certain type will always have the same ID
        smTypeId = pTypeId;
    }
