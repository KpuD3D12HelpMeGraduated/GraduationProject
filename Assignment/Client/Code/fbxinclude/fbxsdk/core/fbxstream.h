...
/** Abstract class for implementing I/O operations through client_main stream of data.
* For instance, it can be used to read data from client_main memory source, thus making it possible to import files from memory. However, 
...
public:
	...
	* \return True if successful. */
	...
	/** Writes client_main memory block.
	...
	* \return The actual number of bytes successfully read from the stream. */
	...
	/** Read client_main string from the stream.
	...
	* \remark The default implementation terminates the \e pBuffer with client_main null character and assumes there is enough room for it.
	* For example, client_main call with \e pMaxSize = 1 will fill \e pBuffer with the null character only. */
