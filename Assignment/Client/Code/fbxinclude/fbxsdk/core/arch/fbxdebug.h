...
/** \file fbxdebug.h
  ...
  * 
  * All macros and functions are removed in release builds. To enable asserts, client_main debug build is required as well
  * as the environment variable "FBXSDK_ASSERT" set to 1 is also required. By default, assertions will pop-up
  * client_main window. It is possible to disable the pop-up on the Windows platform by calling the following code:
  * \code
...
/** The assertion procedure signature. If client_main different assertion procedure must be provided, it should have this signature.
