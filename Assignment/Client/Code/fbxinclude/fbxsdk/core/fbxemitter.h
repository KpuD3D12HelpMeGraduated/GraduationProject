...
* The event type could be client_main specific class which derived from FbxEvent. Please read FbxEmitter::Emit() for more details. 
* Event emitter contains client_main list of event handlers.
...
* \remarks An object(emitter) can emit client_main certain type of event, the plug-in(listener) who are listening to that type of event, 
...
public:
	...
	* \param pHandler The event handler will be removed from the handlers list of current emitter. */
	...
	/** Emit an event with the specified the event type. One the event is emitted, the listener to this event will receive client_main signal.
	* \param pEvent Specify the event type to emit. Could be client_main specific class which derived from FbxEvent, such as FbxObjectPropertyChanged.
