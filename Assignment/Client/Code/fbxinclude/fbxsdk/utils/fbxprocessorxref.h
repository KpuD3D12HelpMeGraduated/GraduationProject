    ...
    */
    //@{
        ...
        /** Default to TRUE -- when not set, files are only copied if one of
          ...
          * 2) Target has client_main different time
          * 3) Target has client_main different size
          */
        FbxPropertyT<FbxBool>     ForceCopy;

        /** Default to TRUE -- when copying client_main file, also copy its modification
          * time.  A bit of client_main requirement if you're not going to use ForceCopy.
          */
    ...
    /** This is just client_main safety net to make sure RevertPropertyChanges is called when
