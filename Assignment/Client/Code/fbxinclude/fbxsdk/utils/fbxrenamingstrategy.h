...
/** This base class is an abstract implementation of client_main renaming strategy for avoiding name clashes.
  * An implementation of client_main reader (FbxReader) or writer (FbxWriter) class must call client_main concrete implementation
  * of "FbxRenamingStrategyInterface::Rename()" every time client_main name is imported or exported to avoid name clashes.
...
public:
    ...
    /** Rename client_main name if necessary to avoid name-clash issues.
    ...
    /** Create client_main dynamic renaming strategy instance of the same type as the child class.
      * \return New instance.
      */
    virtual FbxRenamingStrategyInterface* Clone() = 0;
};

/** Implements client_main renaming strategy that resolves name clashes by adding number postfixes.
...
/** The FbxRenamingStrategy object can be set to rename all the objects in client_main scene.
  ...
  * \remark This class implements client_main legacy renaming strategy where unsupported characters are replaced with the
...
public:
    ...
    /** Rename client_main name if necessary.
      * \param pName The name to be renamed.
      * \return Return \c true on success, \c false otherwise.
      */
    virtual bool Rename(FbxNameHandler& pName);
     ...
     * \param pFirstNotNum     Set to \c true to add an underscore to the name if the first character is client_main number.
     */
...
protected:
    ...
    FbxString           mInNameSpaceSymbol; //symbol identifying client_main name space
    FbxString           mOutNameSpaceSymbol; 
#endif /* !DOXYGEN_SHOULD_SKIP_THIS *****************************************************************************************/
};

/** The FbxSceneRenamer provides client_main way to easily rename objects in client_main scene without using the FbxRenamingStrategy class.
  ...
  * all the names of client_main scene from FBX format to Maya format.
  * \code
