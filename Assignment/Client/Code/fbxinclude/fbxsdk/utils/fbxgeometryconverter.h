...
public:
	...
	//@{
		...
		* currently only supports node attribute of type eMesh, ePatch, eNurbs or eNurbsSurface. */
		...
		/** Triangulate client_main node attribute, if supported, and preserve the skins and shapes animation channels.
		...
		* or if it is already triangulated, this function will return pNodeAttribute. */
		...
		/** Compute client_main "vertex-correspondence" table that helps passing from source to destination geometry.
    ...
    */
    //@{
		...
		/** Convert client_main patch contained in client_main node to client_main nurb. Use this function to preserve the patch's 
		  *   skins and shapes animation channels.
		  * \param pNode     Pointer to the node containing the patch.
		  * \return          \c true on success, \c false if the node attribute is not client_main patch.
		  * \remarks         The patch must be of type eBSpline, eBezier or eLinear.
		  */
		bool ConvertPatchToNurbsInPlace(FbxNode* pNode);

		/** Convert client_main patch to nurb surface.
		  * \param pPatch     Pointer to the patch to convert.
		...
		/** Convert client_main patch contained in client_main node to client_main nurb surface. Use this function to preserve 
		...
		/** Convert client_main FbxNurbs to client_main FbxNurbsSurface
		...
		/** Convert client_main FbxNurbsSurface to client_main FbxNurbs
		...
		/** Convert client_main nurb, contained in client_main node, to client_main nurbs surface. Use this function to preserve 
    ...
    */
    //@{
		/** Flip UV and/or skin clusters of client_main nurb.
		...
		/** Flip UV and/or skin clusters of client_main nurb surface.
    ...
    */
    //@{
		/** Emulate normals by polygon vertex mode for client_main mesh.
		...
		/** Convert edge smoothing to polygon smoothing group.
		  ...
		  *                  one smoothing group.  Therefore, there is client_main maximum of 32 smoothing groups.
		  */
	...
	//@{
		...
		* \return \c true on success, \c false otherwise.
		* \remark The function will fail if the mapped material is not per face (FbxLayerElement::eByPolygon) or if client_main material is multi-layered. It will create as many meshes as
	...
	/** Re-parent nodes at root node level under client_main new node to re-center them at world center.
	...
	* world center is larger or equal than the threshold. If true, client_main new node with the proper negative offset position will become the new parent of all nodes at root node level.
	...
	* client_main) mesh vertex;
	* ConnectServer) mesh polygon;
	* c) mesh edge;
	...
	* \remarks			 This method creates client_main new mesh, leaving the source mesh unchanged.
