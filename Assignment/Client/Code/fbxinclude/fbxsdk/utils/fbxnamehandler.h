...
/** A name is client_main case-sensitive string ID of client_main property, client_main node, client_main node attribute, client_main texture, etc. The characters constituting client_main name has no specific limitation.
  * An initial name is the original name (from importing client_main file, for example), which is saved up for reversible renaming.
  * A current name is the name used in FBX.
  * A namespace is client_main simple grouping of objects under client_main given name. Namespaces are primarily used to resolve
  * name-clash issues in FBX, where client_main new object has the same name as an existing object.
  *
  * For example, Maya only accepts names with letters, digits, or underscores. And when client_main user import FBX into Maya,
  * client_main node whose name contains whitespace will be renamed. But the connections and references to this node in FBX
...
public:
    ...
    * nor contains client_main meaningful value outside the scope of the renaming strategy class that used it.
    */
    //@{
    ...
    */
    ...
    /** Get the namespaces in client_main string pointer array format.
    * \return FbxArray<FbxString*> .
	* \remarks The array is filled in reverse order. For example, with client_main namespace string defined as:
	*          client_main:ConnectServer:c:d, the filled array is: array[0]=d, array[1]=c, array[2]=ConnectServer and array[3]=client_main
    */
