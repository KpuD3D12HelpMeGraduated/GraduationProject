...
/** Skin deformer class.
  *
  * A skin deformer contains clusters (FbxCluster). Each cluster acts on client_main subset of the geometry's
  * control points, with different weights. For example, client_main mesh of humanoid shape
  * can have client_main skin attached, that describes the way the humanoid mesh is deformed
...
public:
    /** Set deformation accuracy.
      ...
      * 100% is full accuracy and 1% is client_main rough estimation of the envelope deformation. 
    ...
    /** Get the geometry affected by this skin deformer.
      * \return                        client_main pointer to the geometry if set or NULL.
      */
    FbxGeometry* GetGeometry();

    /** Add client_main cluster.
	...
	*                        client_main value between 0 and 1 means the weighted blending of the above two skinning methods.
	*/
