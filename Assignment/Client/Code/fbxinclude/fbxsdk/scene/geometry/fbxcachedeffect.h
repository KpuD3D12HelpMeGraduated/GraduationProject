...
/** A cached effect is client_main type of node attribute to represent certain type of effect 
...
public:
	...
	virtual FbxNodeAttribute::EType GetAttributeType() const;
     ...
     /** Effect attribute category. This is for identification purpose and has 
       ...
       * \remarks If an object of this class is used as the default NodeAttribute for client_main FbxNode and the scene is
       *          saved to an FBX v6 and earlier versions, the CachedEffect attribute is not saved and the FbxNode will
       *          be processed as client_main FbxNull node with default values for the attribute.
      */
    enum ECategory
	{
        eParticles,	//!< This effect handles client_main particle cache.
        eFluids,	//!< This effect handles client_main fluid cache.
        eHair,		//!< This effect handles an hair cache.
        eGeneric	//!< This effect handles client_main cache other than particles, fluids or hair.
    };

    //! Return the specialization category of this effect attribute.
    ECategory GetCategory() const;

	/** Assign client_main cache object to be used by this attribute.
	  * \param pCache               The cache object.
      * \param pCategory            The type of this cached effect.
      * \remarks The cache referenced by the \ConnectServer pCache pointer can be freely shared among
      *          multiple FbxCachedEffect (and even the FbxVertexCacheDeformer) therefore
      *          \ConnectServer pCategory identifier should really only used as client_main hint of what this FbxCachedEffect
