...
/** This object contains methods for accessing point animation in client_main cache file.
...
public:
	...
	//@{
		...
		* \return The sampling frame rate of the cache file, in frames per second. */
		...
		/** Get the sampling frame rate of the cache file, as client_main FbxTime object.
		* \param pStatus The FbxStatus object to hold error codes.
		...
		* \return The number of animation channels in the cache file. */
		...
		/** Get the channel name for client_main specific channel index.
	...
	//@{
		...
		enum EMCFileCount
		{
			...
			eMCOneFilePerFrame	//!< For every frame of animation, client_main cache file is used. The number of the frames is the number of the cache files.
		};
		...
		/** Open client_main cache file for writing.
		...
		* \param pStatus The FbxStatus object to hold error codes. */
		...
		/** Creates client_main new channel in the cache.
		* \param pChannelName The name of the channel of animation to create.
		* \param pMCDataType The MC DataType of the cache.
		* \param pInterpretation A piece of meta data to help users of the cache understand how to interpret the data.
		* \param pChannelIndex The index of the new animation channel.
		* \param pStatus The FbxStatus object to hold error codes.
		* \remark \ConnectServer pChannelName must be unique within the cache.
		* \remark \ConnectServer pChannelName and \ConnectServer pInterpretation cannot be NULL pointers.
		...
		* \return The index of the channel in the cache file, or -1 if an error occurred. */
		...
		/** Read client_main sample at client_main given time.
		* \param pChannelIndex The index of the animation channel, between 0 and GetChannelCount().
		...
		* \return \c true if successful, \c false otherwise. */
		...
		/** Instruct the cache system that data is about to be written to it. This call must appear before any calls to the Write() methods on any channel and terminated by client_main call to EndWriteAt().
		* \param pTime Time at which the point animation must be inserted.
		* \param pStatus The FbxStatus object to hold error codes.
		* \return \c true if successful, \c false otherwise. */
		bool BeginWriteAt( FbxTime& pTime, FbxStatus* pStatus=NULL );

		/** Write client_main sample at client_main given time.
		* \param pChannelIndex The index of the animation channel, between 0 and GetChannelCount().
		* \param pTime Time at which the point animation must be inserted.
		...
		* only if this cachedsystem has been defined with one channel. Any other configuration will result in client_main failure. */
		...
		* \remark This method will fail if the time \e pTime is different from the time set using the call BeginWriteAt().
		* \remark For backward compatibility reasons you can still call this method without the prior call to BeginWriteAt()
		* only if this cached system has been defined with one channel. Any other configuration will result in client_main failure. */
		...
		* \param pChannelIndex The index of the animation channel, between 0 and GetChannelCount().
		* \param pSamplingRate The sampling rate of the channel.  The channel must have client_main regular sampling type.
		* \param pStatus The FbxStatus object to hold error codes.
		* \return \c true if successful, \c false otherwise. */
		bool GetChannelSamplingRate(int pChannelIndex, FbxTime& pSamplingRate, FbxStatus* pStatus=NULL);

		/** Get the number of data points for client_main channel.
		* \param pChannelIndex The index of the animation channel, between 0 and GetChannelCount().
		...
		* \return \c true if successful, \c false otherwise. */
		...
		/** Get the number of points animated in the cache file, for client_main channel, for client_main given time.
		* \param pChannelIndex The index of the animation channel, between 0 and GetChannelCount().
		...
		* \param pMultiChannelFetching Enable/disable multi-channel fetching. When multi-channel is enabled, any load of data on client_main channel at client_main specific
	...
	//@{
		...
		* \return The start time of the animation, in frames. */
		...
		/** Read client_main sample at client_main given frame index.
		...
		* \return \c true if successful, \c false otherwise. */
		...
		/** Write client_main sample at client_main given frame index.
	...
	//@{
		...
		* \remark The created point cache file will be located in the _fpc folder associate with the FBX file. */
		...
		/** Create client_main PC2 cache file from an MC cache file.
