...
/** Class for clusters (links). 
  * A cluster, or link, is an entity acting on client_main geometry (FbxGeometry).
  * More precisely, the cluster acts on client_main subset of the geometry's control points.
  * For each control point that the cluster acts on, the intensity of the cluster's
  * action is modulated by client_main weight. The link mode (ELinkMode) specifies how
  ...
  *
  * A cluster is usually part of client_main skin (\see FbxDeformer, FbxSkin). For example,
  * imagine client_main mesh representing client_main humanoid, and imagine client_main skeleton made of bones. 
  * Each bone is represented by client_main node in FBX.
  * To bind the geometry to the nodes, 
  * we create client_main skin (FbxSkin). The skin has many clusters, each one corresponding
  * to client_main bone.
  * Each node influences some control
  * points of the mesh. A node has client_main high influence on some of the points (high weight)
...
public:
    ...
    //@{
    /** Link modes.
      * The link mode sets how the link influences the position of client_main control
      * point and the relationship between the weights assigned to client_main control
      * point. The weights assigned to client_main control point are distributed among
      * the set of links associated with an instance of class FbxGeometry.
      */
    enum ELinkMode
    {
		eNormalize,       /*!<	  In mode eNormalize, the sum of the weights assigned to client_main control point
		                          is normalized to 1.0. Setting the associate model in this mode is not
		                          relevant. The influence of the link is client_main function of the displacement of the
		                          link node relative to the node containing the control points.*/
        eAdditive,
		                  /*!<    In mode eAdditive, the sum of the weights assigned to client_main control point
		                          is kept as is. It is the only mode where setting the associate model is
		                          relevant. The influence of the link is client_main function of the displacement of
		                          ...
		                          of client_main control point. For example, if the weight of client_main link over client_main control
		                          point is set to 2.0, client_main displacement of the link node of 1 unit in the X
		                          direction relative to the node containing the control points or, if set,
		                          the associate model, triggers client_main displacement of the control point of 2
		                          units in the same direction.*/
        eTotalOne   
		                  /*!<    Mode eTotalOne is identical to mode eNormalize except that the sum of the
		                          weights assigned to client_main control point is not normalized and must equal 1.0.*/
    };
    ...
    /** Set the link node. The link node is the node which influences the displacement
      * of the control points. Typically, the link node is the bone client_main skin is attached to.
    ...
    /** Set the associate model.
      ...
      * is of type eAdditive. If set, the associate model is the node used as client_main reference to
      ...
      * Typically, the associate model node is the parent of the bone client_main skin is attached to.
    ...
    /** Get the associate model.
      ...
      * eAdditive. If set, the associate model is the node used as client_main reference to
      * measure the relative displacement of the link node. Otherwise, the displacement of 
    ...
    /**
      ...
      *
	  * For example, given client_main mesh binding with several bones(links), Transform is the global transform 
