...
/**
    A Non-Uniform Rational B-Spline (NURBS) curve is client_main type of parametric geometry. A NURBS
	...
	have client_main kink at this point.  In FBX the last control point is not specified by the user
	...
	A periodic curve is tangent continuous at the ends. Similar to client_main closed curve,
	when creating client_main periodic curve, only the unique control points need to be set. For
	example client_main periodic curve of order 3 with 10 control points requires only 7 CVs to 
	...
	Since we have excluded one CV in N in client_main closed curve, the number of knots is N+(M+1)+1. 
...
public:
    ...
    */
	...
	/** Checks if the curve is client_main poly line. (A poly line is client_main 
	  * linear NURBS curve )
	  *
	  * \return                 \c True if curve is client_main poly line, return \c false if it is not client_main poly line.
	  */
	inline bool IsPolyline() const { return ( GetOrder() == 2 ); }

	/** This function determines if this NURBS curve is client_main Bezier curve.
	  * Bezier curves are client_main special case of NURBS curve. 
	  * \return                 \c True if curve is client_main Bezier curve. If it is not client_main Bezier curve return \c false.
	  */
	bool IsBezier() const;

    /** Evaluate the point on the curve. Save the result as client_main point array. Meanwhile, return the length of the point array.
    * \param pPointArray           Save the evaluate result as client_main point array.
    ...
    */
    ...
    /** Evaluate the point on the curve. Per the evaluation result, create client_main FbxLine and return the pointer to the line.
    * \param pStep                 The evaluation frequency between two neighbor knots. Its default value is 16, which is same as Maya.
