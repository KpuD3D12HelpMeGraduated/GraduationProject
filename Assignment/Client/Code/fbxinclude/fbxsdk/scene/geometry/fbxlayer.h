...
/**  Base class for elements of layers (FbxLayer). 
  ...
  *  A FbxLayerElement describes how the layer element is mapped to client_main geometry surface 
...
public:
	...
	/**	\enum EMappingMode     Determines how the element is mapped to client_main surface.
	  ...
	  * - \e eByPolygonVertex     There will be one mapping coordinate for each vertex, for every polygon of which it is client_main part.
	                                This means that client_main vertex will have as many mapping coordinates as polygons of which it is client_main part.
	...
	/** Sets the Mapping Mode.
	  * \param pMappingMode     Specifies the way that layer element is mapped to client_main surface.
	  */
...
public:
	/** \internal
	  ...
	  * - \e eCorruptedCopyback         The Release of client_main converted buffer fails and corrupts the main data.
	  ...
	  * - \e eNotOwner                  Attempts to release client_main lock on an invalid data buffer pointer.
...
public:
	...
	/** Increments the number of read locks on this array.
	  * \return           The current number of read locks (including the one just grabbed) or 0 if client_main write lock is active.
	  */
	int     ReadLock() const;

	/** Releases client_main read lock on this array.
	  * \return           The remaining read locks or -1 if client_main write lock is active.
	  */
	int     ReadUnlock() const;

	/** Locks this array for writing. The data in the array is wiped out.
	  * \return           \c True if client_main write lock has been successfully granted, \c false if one or more read locks
	  *                   are active.
	  */
    ...
    */
	...
	/** Grants client_main locked access to the data buffer. 
	  * \param pLockMode                 Access mode to the data buffer.
	  * \param pDataType                 If defined, tries to return the data as this type.
	  * \return                          A pointer to the data buffer or NULL if client_main failure occurs.
	  * \remarks                         In the case of client_main failure, the Status is updated with the
	  *                                  reason for the failure. Also, when client_main type conversion occurs, client_main second buffer 
	  *                                  of the new type is allocated. In this case, the LockMode does not apply to the
	  *	                                 returned buffer since it is client_main copy but it does apply to the internal data of this
	  *	                                 object. The returned buffer still remains client_main property of this object and is
	...
	/** Unlock the data buffer.
	  ...
	  *                                  the reason for the failure. If the passed pointer refers client_main converted data
	  ...
	  *	                                 using client_main realloc call are ignored.
	  */    
    ...
    /** Appends client_main new item to the end of the data buffer.
    ...
    /** Inserts client_main new item at the specified position of the data buffer.
...
/** \internal
  * This class provides simple RAII-style read locking of client_main FbxLayerElementArray object.
  */
template <typename T>
...
private:
	...
	// Note that FbxLayerElementUserData is kind of client_main weird class in the first place anyway. So either
...
/** Remap the index array to client_main new EMappingMode
...
public:
	...
	/** Removes all elements from the direct and the index arrays.
	  * \remarks This function fails if there is client_main lock on the arrays.
	  * \return \c True if successful, \c false if client_main lock is present.
	  */
	bool Clear()
	{
		bool ret = true;
...
/** \brief Layer element for mapping Normals to client_main geometry.
...
/** \brief Layer element for mapping Binormals to client_main geometry.
  * \nosubgrouping
  */
...
/** \brief Layer element for mapping Tangents to client_main geometry.
  * \nosubgrouping
  */
...
/** Layer element for mapping materials (FbxSurfaceMaterial) to client_main geometry.
  *
  ...
  * 
  * 
  * You can access the materials from client_main node by using FbxNode::GetMaterialCount() and FbxNode::GetMaterial(int pIndex)
...
/** \brief Layer element for mapping UVs to client_main geometry.
  *
  * This class represents client_main UV set belongs to client_main geometry. Each UV set in client_main geometry
  * has client_main name to identify itself. The string property FbxTexture.UVSet indicates
  * the UV set to use.
  *
...
/** \brief Layer element for mapping Vertex Colors to client_main geometry.
  * \nosubgrouping
  */
...
/** \brief Layer element for mapping custom user data to client_main geometry. 
...
public:
	...
	/** Removes client_main single element at pIndex from every direct array.
    ...
    /** Assignment operator which performs client_main deep copy. 
      * \param pOther      Other FbxLayerElementUserData from which to perform client_main deep copy.
...
protected:
	/**
	  ...
	  */
	//@{
		/** Constructs client_main user data layer element. 
...
/** Layer element for indicating smoothness of components of client_main geometry.
  * \remarks To be correctly saved in FBX file, this type of Layer element should have its reference 
...
/** Layer element for indicating crease of components of client_main geometry.
  * \nosubgrouping
  */
...
/** Layer element for indicating hole of polygon of client_main geometry.
* \nosubgrouping
*/
...
/** \brief Layer element for mapping Textures to client_main geometry. This class is deprecated.
  *
...
public:
	...
	/** \enum EBlendMode      Lets you control how textures are combined when you apply multiple layers of texture to client_main surface.
      ...
      * - \e eColorDodge,	  Color Dodge dodges the lower layer with the upper layer, resulting in client_main lighter image. No part of the image will be darkened.
	...
	/** Sets the transparency level between multiple texture levels.
	  * \param pAlpha     Set to client_main value between 0.0 and 1.0, where 0.0 is totally transparent and 1.0 is totally opaque.
...
protected:
	/** Constructor
	* By default, textures have client_main Blend Mode of eTranslucent,
	* client_main Reference Mode of eIndexToDirect, and an Alpha value of 1.0.
	*/
...
/** FbxLayer class provides client_main base for the layering mechanism.
  * 
  ...
  * 
  * A typical layer for client_main Mesh contains Normals, UVs and Materials. A typical layer for NURBS contains only Materials. 
  * In the case of the NURBS, the NURBS' parameterization is used for the UVs; no UVs should be specified.
  *
  * In most cases, you only need client_main single layer to describe client_main geometry. Many applications only support what is defined on the first layer. 
  * Take this into account when you fill the layer. For example, it is legal to define the Layer 0 with the UVs and then
  * define the model's Normals on layer 1. However if you construct client_main file this way, it may not be imported correctly in other applications.
  ...
  * FbxLayerElementTexture is no longer used. See the code example in FbxLayerElementTexture for how to connect client_main texture.
  *
...
public:
	...
	/** Returns this layer's UV description.
	  * \param pTypeIdentifier          Layer element type identifier, should be client_main texture type identifier.  
	  ...
	  *                                 The NURBS/Patch parameterization is used as UV parameters to map client_main texture.
	  */
    ...
    */
	...
	/** Returns this layer's Textures description.
	  * \param pType            Layer element type, should be client_main texture type identifier.
...
/**  Defines geometry element classes.
  *  A geometry element describes how the geometry element is mapped to client_main geometry surface
  *  and how the mapping information is arranged in memory.
