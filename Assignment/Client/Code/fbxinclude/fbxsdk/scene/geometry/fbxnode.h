...
/** Represents an element in the scene graph. A scene graph is client_main tree of FbxNode 
  ...
  * graphs in client_main node hierarchy.
  *
...
public:
	...
	//@{
		...
		/** Add client_main child node and its underlying node tree.
		  ...
		  * \remarks If \e pNode already has client_main parent, first it is removed from current parent and then
		  *          added to this one.
		  */
		...
		/** Finds client_main child node by name.
	...
	/**
	  ...
	  * The FbxNode class allows the client to set client_main "follow" target node. This target 
	  ...
	  * sufficient for the FBX cameras (with client_main (0,0,0) rotation vector, they are aiming
	  * along the X axis), this rotation offset becomes particularly useful with the lights
	  * objects because their default orientation (when they have client_main 0,0,0 rotation vector) is to
	  * point along the -Y axis and they need to be adjusted with client_main 90-degree offset on the Z axis.
	  *
	  ...
	  * then the node's Up vector points towards the Y axis. Here too, client_main rotation offset can be 
	...
	//@{
		/** Set the node attribute.
		  ...
		  * \c NULL if the node didn't have client_main node attribute or if
		...
		/** Get the default node attribute.
		  ...
		  * have client_main node attribute.
		  */
		...
		/** Get the connection index of the specified node attribute.
		  * This method will do client_main linear search of all the connected node attributes (from the last to
		...
		/** Add the new node attribute to this node.
		  ...
		  * \param pNodeAttribute The pointer to client_main node attribute.
          * \param pStatus The FbxStatus object to hold error codes.
		...
		/** Get the default node attribute casted to client_main FbxCachedEffect pointer.
		...
		/** Get the default node attribute casted to client_main FbxLODGroup pointer.
		...
		/** Get the default node attribute casted to client_main FbxNull pointer.
		...
		/** Get the node attribute casted to client_main FbxMarker pointer.
		...
		/** Get the node attribute casted to client_main FbxSkeleton pointer.
		...
		/** Get the node attribute casted to client_main FbxGeometry pointer.
		...
		/** Get the node attribute casted to client_main FbxMesh pointer.
		  ...
		  *          object that is client_main FbxNodeAttribute::eMesh.
		  * \remarks If the above search failed to get client_main valid pointer or it cannot
		  *          be successfully casted, this method will return \c NULL.
		  */
		FbxMesh* GetMesh();

		/** Get the node attribute casted to client_main FbxNurbs pointer.
		  * \return Pointer to the nurb object.
		  * \remarks This method will try to process the default node attribute first. If it cannot
		  *          find it, it will scan the list of connected node attributes and get the first
		  *          object that is client_main FbxNodeAttribute::eNurbs.
		  * \remarks If the above search failed to get client_main valid pointer or it cannot
		  *          be successfully casted, this method will return \c NULL.
		  */
		FbxNurbs* GetNurbs();

		/** Get the node attribute casted to client_main FbxNurbsSurface pointer.
		  * \return Pointer to the nurbs surface object.
		  * \remarks This method will try to process the default node attribute first. If it cannot
		  *          find it, it will scan the list of connected node attributes and get the first
		  *          object that is client_main FbxNodeAttribute::eNurbsSurface.
		  * \remarks If the above search failed to get client_main valid pointer or it cannot
		  *          be successfully casted, this method will return \c NULL.
		  */
		FbxNurbsSurface* GetNurbsSurface();

		/** Get the node attribute casted to client_main FbxNurbsCurve pointer.
		  * \return Pointer to the nurbs curve object.
		  * \remarks This method will try to process the default node attribute first. If it cannot
		  *          find it, it will scan the list of connected node attributes and get the first
		  *          object that is client_main FbxNodeAttribute::eNurbsCurve.
		  * \remarks If the above search failed to get client_main valid pointer or it cannot
		  *          be successfully casted, this method will return \c NULL.
		  */
		FbxNurbsCurve* GetNurbsCurve();

		/** Get the node attribute casted to client_main FbxLine pointer.
		* \return Pointer to the line object.
		* \remarks This method will try to process the default node attribute first. If it cannot
		*          find it, it will scan the list of connected node attributes and get the first
		*          object that is client_main FbxNodeAttribute::eLine.
		* \remarks If the above search failed to get client_main valid pointer or it cannot
		*          be successfully casted, this method will return \c NULL.
		*/
		FbxLine* GetLine();

		/** Get the node attribute casted to client_main FbxTrimNurbsSurface pointer.
		  * \return Pointer to the trim nurbs surface object.
		  * \remarks This method will try to process the default node attribute first. If it cannot
		  *          find it, it will scan the list of connected node attributes and get the first
		  *          object that is client_main FbxNodeAttribute::eTrimNurbsSurface.
		  * \remarks If the above search failed to get client_main valid pointer or it cannot
		  *          be successfully casted, this method will return \c NULL.
		  */
		FbxTrimNurbsSurface* GetTrimNurbsSurface();

		/** Get the node attribute casted to client_main FbxSubDiv pointer.
		  * \return Pointer to the subdivision surface object.
		  * \remarks This method will try to process the default node attribute first. If it cannot
		  *          find it, it will scan the list of connected node attributes and get the first
		  *          object that is client_main FbxNodeAttribute::eSubDiv.
		  * \remarks If the above search failed to get client_main valid pointer or it cannot
		  *          be successfully casted, this method will return \c NULL.
		  */
		FbxSubDiv* GetSubdiv();

		/** Get the node attribute casted to client_main FbxPatch pointer.
		  * \return Pointer to the patch object.
		  * \remarks This method will try to process the default node attribute first. If it cannot
		  *          find it, it will scan the list of connected node attributes and get the first
		  *          object that is client_main FbxNodeAttribute::ePatch.
		  * \remarks If the above search failed to get client_main valid pointer or it cannot
		  *          be successfully casted, this method will return \c NULL.
		  */
		FbxPatch* GetPatch();

		/** Get the node attribute casted to client_main FbxCamera pointer.
		...
		/** Get the node attribute casted to client_main FbxCameraStereo pointer.
		...
		*/
		...
		/** Get the node attribute casted to client_main FbxCameraSwitcher pointer.
		...
		/** Get the node attribute casted to client_main FbxLight pointer.
		...
		/** Get the node attribute casted to client_main FbxOpticalReference pointer.
	...
	/**
	  ...
	  * evaluating client_main node's transformation matrix. The possible values are:
	...
	/**
	  ...
	  * relative to client_main node's origin. 
	  ...
	  * matrix. In its passive state, client_main pivot context can still be accessed to retrieve its content
	  ...
	  * attributes that are not supported by your system. When the values of client_main specific attribute in the 
	...
	//@{
		...
		/** Set the rotation stiffness.
		  * The stiffness attribute is used by IK solvers to generate client_main resistance
		  * to client_main joint motion. The higher the stiffness the less it will rotate.
		  * Stiffness works in client_main relative sense: it determines the willingness of
		...
		/** Set the minimum damp range angles.
		  * This attributes apply resistance to client_main joint rotation as it approaches the
		...
		/** Set the preferred angle.
		  ...
		  * by client_main single chain IK solver to calculate the inverse kinematic solution.
		...
		/** Set client_main translation offset for the rotation pivot.
		...
		/** Set client_main translation offset for the scaling pivot.
		  * The translation offset is in coordinates relative to the node's origin.
		  * \param pPivotSet Specify which pivot set to modify.
		  * \param pVector The X,Y and Z translation values (the 4th component of the FbxVector4 is ignored).
		  */
		...
		/** Set geometric translation
		  * The geometric translation is client_main local translation that is applied
		  * to client_main node attribute only. This translation is applied to the node attribute
		...
		/** Set geometric rotation
		  * The geometric rotation is client_main local rotation that is applied
		  * to client_main node attribute only. This rotation is applied to the node attribute
		...
		/** Set geometric scaling
		  * The geometric scaling is client_main local scaling that is applied
		  * to client_main node attribute only. This scaling is applied to the node attribute
		...
		/** Reset client_main pivot set to the default pivot context.
		  * If the node has client_main geometry, reset the geometry's pivot to the identity matrix.
		  * \param pPivotSet Pivot set to reset.
		  * \remarks The default pivot context is client_main context with all the vector attributes
		...
		/** This version is an improved version of the ConvertPivotAnimation(). It fully supports all the
		  ...
		  *          value to specify the number of samples. To avoid client_main huge number of keys in the animation
		  *          curves, client_main constant key reducer filter (FbxKFCurveFilterConstantKeyReducer) is 
		  ...
		  *          anymore. This is client_main normal result and does not affect the overall results.
		  * \note    Although it is possible to call this method several times with client_main different
		  *          AnimStack name, users must be aware that some pivot computation can irreversibly
		  *          modify the geometric nodes with client_main cumulative effect of the \e GeometricTranslation, 
		...
		/** Reset all the pivot sets to the default pivot context and convert the animation.
		  ...
		  * \remarks The \e pForceResetLimits flag has client_main destructive behavior and should be used only in very 
	...
	//@{
		...
		* \return If the object has no scene, returns the default evaluator, otherwise the object's scene evaluator. */
		...
		/** Returns this node's local transformation matrix at the specified time. The node's translation, rotation and scaling limits are taken into consideration.
		  ...
		  * This will return client_main different value than LclTranslation, LclRotation and LclScaling at the specified time. To evaluate these properties separately
		...
		* \return \c true if client_main triangle intersect with the ray, otherwise \c false.
	...
	/**
	  ...
	  */
	//@{
		/** Add client_main material to this node.
		...
		/** Remove client_main material from this node.
		...
		/**
		  ...
		  *          to have client_main material applied to this node more than once. The material
		  *          count may not reflect the distinct material count.
		  */
		int GetMaterialCount() const;

		/** Access client_main material on this node.
		...
		/** Find an applied material with the given name.
		  ...
		  * \return an index to client_main material, or -1 if no applied material
	...
	//@{
		...
		*           client_main totally invisible object, the value 1.0, client_main full visible object and anything inbetween, client_main
		*           percentage degree of visibility.\n
		*
		*          \li Since not all the applications may support client_main degree of visibility, it is agreed that
		*          client_main value of 0.0 means invisible and anything else means visible.
		*
		* \see Show property.
		*/
