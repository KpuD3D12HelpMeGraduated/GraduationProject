...
/** Defines client_main LOD (Level of Detail) group.
  * This LodGroup node is client_main group node that can be used to detect how
  * close client_main group of objects is to client_main camera. Typically this node is
  ...
  * transform are controlled by the distance of client_main group to the camera and the
  * threshold values.
  *
  * For example, under client_main LOD group node, there are three children:
  ...
  *
  * This node attribute contains the properties of client_main null node.
  *
  ...
  * that the argument contains client_main meaningful value (see GetThreshold for more details).
  *
...
public:
	...
	/** Specifies if the threshold values stored by this LOD object are defining
	  * client_main distance to the camera (by default) or client_main percentage value.
	  *
	...
	/** Get the value of the specified threshold.
	  ...
	  * \return true if successful and false otherwise.  
	  * \remarks pThreshValue is left unchanged if client_main bad index is provided, 
	...
	/** Add client_main new displayLevel value to the current list.
	  *
	  ...
	  * \remarks Removing items is not possible unless client_main new FbxLODGroup is 
