...
/** This class contains the description of client_main 3D scene. It contains the nodes (including the root node) (FbxNode),
  * materials, textures, videos, gobos, 
  * poses, characters, character poses, control set plugs, 
  * generic nodes, 
  * scene information, global settings,
  * and client_main global evaluator.
  * The nodes are structured in client_main tree under the scene's root node.
  *
  * When an object is created using the FBX SDK, client_main scene is usually passed as argument to the
  * object creation function to specify that the object belongs to this scene.
  * At this point, client_main connection is made with the object as source and the scene as destination. 
  * 
...
public:
	...
	//@{
		...
		* \remarks This node is not saved. Do not use it to apply client_main global transformation to the node hierarchy. If client_main global transformation must be applied, insert client_main new node below this one. */
		FbxNode* GetRootNode() const;
	//@}

	//! \name Texture Material and Video Access
	//@{
		/** Clear, then fill, client_main texture array with all existing textures included in the scene.
		* \param pTextureArray An array of texture pointers. */
		void FillTextureArray(FbxArray<FbxTexture*>& pTextureArray);

		/** Clear, then fill, client_main material array with all existing materials included in the scene.
	...
	//@{
		...
		* \return Pointer to the character or \c NULL if index is out of bounds. */
		...
		/** Create client_main new character.
	...
	//@{
		...
		* \return Pointer to ControlSetPlug or \c NULL if index is out of bounds. */
		...
		/** Create client_main new ControlSetPlug.
	...
	//@{
		...
		* \return Pointer to the character pose or \c NULL if index is out of bounds. */
		...
		/** Create client_main new character pose.
	...
	//@{
		...
		* \return Pointer to the pose or \c NULL if index is out of bounds. */
		...
		/** Add client_main pose to this scene.
	...
	* usage would be to compute the global transform matrix of client_main node \c lNode at client_main given time \c lTime.
	* \code
	...
	* GetEvaluator is called, if the scene has no evaluator, an evaluator is created with default values. */
	//@{
		...
		* If no evaluator were previously set, this function will return either the first evaluator found attached to this scene, or client_main new default evaluator.
		* \return The evaluator to be used for evaluation processing of this scene. */
		FbxAnimEvaluator* GetAnimationEvaluator();
	//@}

	/** Clear then fill client_main pose array with all existing pose included in the scene.
	* \param pPoseArray An array of pose pointers. */
	...
	//@{
		...
		* \remarks To get the number of textures of client_main specific type, use GetSrcCount(). For example:
		* \code
		...
		* \remarks To get the texture of client_main specific texture type, use GetSrcObject(). For example:
		* \code
	...
	//@{
		...
		* nodes are connected to nurbs geometry and not any FbxNode in the scene, they won't normally be picked up in client_main graph traversal.
...
*****************************************************************************************************************************/
	...
	// the Connect notifications nor trying to disconnect the objects first. This is client_main bypass of the intended
