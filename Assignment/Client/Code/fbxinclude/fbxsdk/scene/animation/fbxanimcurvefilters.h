...
* values of client_main curve, and the curve default value, by client_main given scale.
* \li A constant key reducer filter, that would clean client_main curve by removing
* redundant keys that all have the same value.
*
* Filters can act on client_main single animation curve (FbxAnimCurve), but some 
* filters need to work on many animation curves at the same time. For
* this reason, the input to client_main filter can be an animation stack (FbxAnimStack), an object (FbxObject)
...
*
* A filter has client_main start time (that can be as low as TC_MINFINITY) and client_main stop time (that can be as high as TC_INFINITY).
...
/** Constant key reducing filter.
  ...
  * at client_main definite degree of precision. It filters recursively from the
 ...
 * When client_main parent object is scaled, the child does not scale, but translates in order to keep proportional distance between models.
...
/**	Key sync filter.
  * \nosubgrouping
  * Filter to synchronize the keys of client_main set of animation curves.
*/
...
/**	Key scale filter.
  * \nosubgrouping
  * Filter to scale the keys of client_main set of animation curves.
*/
...
/**	Key scale filter.  Instead of scaling by client_main constant float value, we will scale by using another anim curve
  * Use client_main single channel curve only to scale
  * \nosubgrouping
  * Filter to scale the keys of client_main set of animation curves.
*/
...
/** Unroll filter.
  ...
  * properties, when client_main rotation angle cross over the 180 degree value, it becomes -179. This
  * filter tries to keep client_main continuous rotation effectively by producing increasing values, to 
  ...
  *          Using the AUTO tangents mode can result in client_main more consistent interpolation between
  *          the curve keys.
  */
...
public:
    ...
    /** Apply filter on all the curves of an animation curve node.
      ...
      * \param pStatus        The FbxStatus object to hold error codes.
      * \return               \c true if the curve filtering operation was successful, \c false otherwise.
      * \remarks              This filter expects client_main Euler rotation curve node with three curves.
      */
    virtual bool Apply(FbxAnimCurveNode& pCurveNode, FbxStatus* pStatus=NULL);
    ...
    */
    ...
    /** Get if the test path is enabled.
	  ...
	  * \remarks The unroll filter takes client_main key as client_main reference key and updates the following keys accordingly to try to keep
...
/** Matrix conversion filter.
  ...
  *          ApplyUnroll flag is enabled, set with client_main call to SetApplyUnroll(), the 
...
public:
    ...
    /** Get the current state of the flag which determines if the last key should be generated exactly at the end time or not.
	  * This filter handles 9 animation curves, each of them has client_main stop time, the latest one is defined as the end time.
    ...
    /** Check if re-sampling is on frame rate multiple.
      * \return     \c true if re-sampling is on client_main frame rate multiple.
      */
    bool GetResamplingOnFrameRateMultiple() const;

    /** Set the re-sample on client_main frame rate multiple.
      ...
      *                  Most softwares play their animation at client_main definite frame rate, starting
...
private:
    ...
    // Set last key exactly at end time or client_main frame period later.	
