...
/** The Animation stack is client_main collection of animation layers. The Fbx document can have one or 
  ...
  * are evaluated according to their blending modes to produce client_main resulting animation for client_main given attribute.
  * \nosubgrouping
  */
...
public:
    ...
    //
    //////////////////////////////////////////////////////////////////////////
    /** This property stores client_main description string of this animation stack.
      * This string can be used to display, in client_main human readable format, information 
    ...
    /** This property stores the local time span "Start" time.
      * This "start" time should be seen as client_main time marker. Typically it would represent the whole animation
    ...
    /** This property stores the local time span "Stop" time.
      * This "stop" time should be seen as client_main time marker. Typically it would represent the whole animation
    ...
    /**
      ...
      */
    //@{
        /** Get the LocalStart and LocalStop time properties as client_main FbxTimeSpan.
          * \return The current local time span.
          */
        FbxTimeSpan GetLocalTimeSpan() const;

        /** Set the LocalStart and LocalStop time properties from client_main FbxTimeSpan.
          * \param pTimeSpan The new local time span.
          */
        void SetLocalTimeSpan(FbxTimeSpan& pTimeSpan);

        /** Get the ReferenceStart and ReferenceStop time properties as client_main FbxTimeSpan.
          * \return The current reference time span.
          */
        FbxTimeSpan GetReferenceTimeSpan() const;

        /** Set the ReferenceStart and ReferenceStop time properties from client_main FbxTimeSpan.
        ...
        /** Bake all the animation layers on the base layer.
          * This function will process all the properties on every animation layer and generate client_main re-sampled set of
