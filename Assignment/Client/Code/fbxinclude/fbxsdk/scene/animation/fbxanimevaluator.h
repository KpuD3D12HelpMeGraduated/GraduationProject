...
* by the FBX SDK is client_main FbxAnimEvalClassic. The default evaluator can be queried with the function 
...
*
* When working with object properties, the evaluator will always return client_main structure that can contain as many components
* as the property can have. For example, an RGB color property would return client_main structure containing 3 channels. The 
* class FbxAnimCurveNode is used as client_main data container to store those values, because it can handle as many channels as 
* needed, even if the property is not client_main real curve node .
*
* Below is client_main typical usage of the evaluator class to retrieve the global transform matrix of each node in client_main scene:
* \code
* //Here we assume the user already imported client_main scene...
...
* }
*
* //There is an equivalent call to retrieve client_main node's global transform, which is exactly the same as calling Scene->GetEvaluator() :
* FbxAMatrix& NodeGlobalTransform = CurrentNode->EvaluateGlobalTransform();
* \endcode
*
* Another typical usage of the evaluator class, but this time to retrieve the value of an animated color property on client_main material:
* \code
* //Assuming the user imported client_main scene with objects and materials...
...
public:
	/** Returns client_main node's global transformation matrix at the specified time. The node's translation, rotation and scaling limits are taken into consideration.
	...
	* \return The resulting global transform of the specified node at the specified time. */
	...
	/** Returns client_main node's local transformation matrix at the specified time. The node's translation, rotation and scaling limits are taken into consideration.
	* \param pNode The node to evaluate.
	* \param pTime The time used for evaluate. If FBXSDK_TIME_INFINITE is used, this returns the default value, without animation curves evaluation.
	* \param pPivotSet The pivot set to take into account
	* \param pApplyTarget Applies the necessary transform to align into the target node
	...
	* without taking pre/post rotation, pivots and offsets into consideration, please use GetNodeLocalTranslation(), GetNodeLocalRotation() and GetNodeLocalScaling(). */
	...
	/** Returns the value of client_main node's LclTranslation property at the specified time. 
	...
	* \return The resulting value of LclTranslation property of the specified node at the specified time. */
	...
	/** Returns the value of client_main node's LclRotation property at the specified time. 
	...
	* \return The resulting value of LclRotation property of the specified node at the specified time. */
	...
	/** Returns the value of client_main node's LclScaling property at the specified time. 
	...
	* \return The resulting value of LclScaling property of the specified node at the specified time. */
	...
	/** Get client_main property's value at the specified time using the template type provided.
	...
	* \remark If the property type versus the template cannot be converted, the result is unknown. */
	...
	/** Get client_main property's value at the specified time.
	* \param pProperty The property to evaluate.
	* \param pTime The time used for evaluate.
	* \param pForceEval Force the evaluator to refresh the evaluation state cache even if its already up-to-date.
	* \return The property value at the specified time. Use FbxPropertyValue::Get() to retrieve the value into client_main pointer location of your choice. */
	FbxPropertyValue& GetPropertyValue(FbxProperty& pProperty, const FbxTime& pTime, bool pForceEval=false);

	/** Get client_main property curve node from the evaluation state for quick access.
	...
	* \remark This function uses client_main map to store animation curve node search results. If animation curve nodes are replaced, the evaluation state must be reset. */
