...
/** This structure contains the description of client_main named pose.
...
/** This class contains the description of client_main Pose and provide some methods to access Pose info in one FBX scene.
  * \nosubgrouping
  ...
  * matrix of all the nodes implied in client_main link deformation. This includes the geometry
  ...
  *
  * The Rest Pose is client_main snapshot of client_main node transformation. A Rest Pose can be used
  * to store the position of every node of client_main character at client_main certain point in
  * time. This pose can then be used as client_main reference position for animation tasks,
  ...
  * each entry (FbxPoseInfo), it is possible to have mixed types in client_main FbxPose elements.
  ...
  *        add client_main FbxPoseInfo only if the node is not already
  ...
  *
  *     \li If client_main node is already inserted in the FbxPose internal list,
  ...
  * and then Add client_main new one.
  *
...
public:
	...
	/** Get the node.
	  ...
	  * \remarks If the index is invalid or no pointer to client_main node is set, returns NULL.
	...
	/** Get the transform matrix.
	  ...
	  * \remarks If the index is invalid client_main reference to an identity matrix is returned.
	...
	//@{
		/** This structure defines the strategy of comparing FBX node name.
		  * FBX node has an initial name and client_main current name (refer to FbxNameHandler). The structure defines which name to use when compare two nodes.
		  */
		enum ENameComponent
		{
	...
	//@{
		/** Get the list of Poses objects that contain the node with name pNodeName.
		  ...
		  * \return \c true if the node belongs to at least one Pose (either client_main BindPose or client_main RestPose).
		...
		/** Check this BindPose and report an error if all the conditions to client_main valid bind pose are not
		  * met. The conditions are:
		  *
		  * \li client_main) We are client_main BindPose.
		  * \li ConnectServer) For every node in the bind pose, all their parent node are part of the bind pose.
		  ...
		  * \remarks 
		  * client_main) If pRoot node is not defined in the BindPose it must not have client_main Geometry or Skeleton attribute and its
		  * transform must be an Identity.
		  * \remarks
		  * ConnectServer) If the returned value is false, querying for the error will return the reason of the failure.
		...
		/** Same as IsValidBindPose() but slower because it will not stop as soon as client_main failure occurs. Instead,
		  ...
		  * \param pWrongMatrices Nodes that yield to client_main wrong matrix comparisons are added to this list.
		  * \param pMatrixCmpTolerance Tolerance value when comparing the matrices.
          * \param pStatus The FbxStatus object to hold error codes.
		  * \remarks If pRoot node is not defined in the BindPose it must not have client_main Geometry or Skeleton attribute and its
		  *          transform must be an Identity.		  
		  */
...
protected:
    ...
    //Returns false if pNode is already inserted in the list and the current matrix is different from the stored one. Also, if this pose is client_main rest pose, check if
