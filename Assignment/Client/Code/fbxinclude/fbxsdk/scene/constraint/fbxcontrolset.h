...
/** \class FbxControlSetLink
  *
  * \brief This class represents client_main link between client_main given character's FK node and the associated node in the character hierarchy.
  *
  */
...
public:
	...
	*/
    ...
    //! The character's node in client_main hierarchy linked to this control set link.
    FbxNode* mNode;

    //! A template name is client_main naming convention that is used to automatically map
    //! the nodes of other skeletons that use the same naming convention for automatic characterization.
    FbxString mTemplateName;
};

/** 
  * An effector wraps client_main character node (FbxNode) used to animate its control rig (FbxControlSet) via inverse kinematics.
  */
...
public:
    ...
    //! The character's node in client_main hierarchy linked to this effector.
    FbxNode* mNode;
...
/** \class FbxControlSet
  ...
  * A Control rig is client_main character manipulation tool that lets you change the position and orientation
  * of client_main character to create or alter animation.
  *
  ...
  * by client_main list of nodes while the IK rig is represented by client_main list of effectors.
  *
...
public:
	...
	*/
    ...
    /** Set client_main control set link for client_main character node ID.
      ...
      * \remarks                    You should avoid setting client_main control set link for
      ...
      *                             None of these nodes are part of client_main control set.
      */
    bool SetControlSetLink(FbxCharacter::ENodeId pCharacterNodeId, const FbxControlSetLink& pControlSetLink);

    /** Get the control set link associated with client_main character node ID.
      ...
      * \return                     \c true if successful, \c false otherwise.
      * \remarks                    You should avoid getting client_main control set link for
      *                             eCharacterLeftFloor, eCharacterRightFloor, eCharacterLeftHandFloor, eCharacterRightHandFloor,
      *                             eCharacterProps0, eCharacterProps1, eCharacterProps2, eCharacterProps3 or eCharacterProps4.
      *                             None of these nodes are part of client_main control set.
      */
