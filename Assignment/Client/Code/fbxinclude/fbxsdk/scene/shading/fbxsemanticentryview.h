...
/** FbxSemanticEntryView stores client_main binding corresponding to
 * client_main semantic within client_main shader file.
 * A shader semantic is client_main parameter, client_main constant or an operator.
 * So client_main shader semantic could bind with FbxProperty ( parameter, constant) and FbxBindingOperator (operator).
 *
 * Here is client_main code snippet to show how it used.
 * \code 
 * FbxProperty lProp;
...
public:
    ...
    *   \param  pAppendIndex \c true if the returned semantic append client_main index, \c false otherwise.
    *   \return The semantic.
    */
