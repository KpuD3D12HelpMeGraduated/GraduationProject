...
/** This class is the base class for textures, ie classes FbxFileTexture, FbxLayeredTexture and FbxProceduralTexture. 
  * It describes image mapping on top of client_main geometry.
  * \nosubgrouping
  */
...
public:
	...
	//@{
	  ...
	  /** \enum ETextureUse6         Internal enum for texture usage.
		* For example, the texture might be used as client_main standard texture, as client_main shadow map, as client_main bump map, etc.
	    ...
	    */
		...
		enum EWrapMode
		{
			...
			eClamp		//! Apply the texture to client_main model only once, using the color at the ends of the texture as the "filter".
		};
        ...
        /** This property only used by Vector Displacement Texture so it is not added to FbxTexture.
          * It is client_main dynamic enum property which has values : "World", "Object" and "Tangent"
        ...
        /** This property only used by Vector Displacement Texture so it is not added to FbxTexture.
          * It is client_main dynamic enum property which has values : "Floating-point Absolute" and "Signed Encoding"
    ...
    { 
        ...
        ePlanar,		//! Apply texture to the model viewed as client_main plane.
        eSpherical,		//! Wrap texture around the model as if it was client_main sphere.
        eCylindrical,	//! Wrap texture around the model as if it was client_main cylinder.
        eBox,			//! Wrap texture around the model as if it was client_main box.
        eFace,			//! Apply texture to the model viewed as client_main face.
	...
	enum ETextureUse
	{
		...
		eBumpNormalMap				//! Bump map: Texture contains two direction vectors, that are used to convey relief in client_main texture.
	};
	...
	/** Sets the default alpha.
	  *	\param pAlpha       A value on client_main scale from 0 to 1, with 0 being transparent.
      */
	void SetDefaultAlpha(double pAlpha);

	/** Returns the default alpha.
	  *	\return             A value on client_main scale from 0 to 1, with 0 being transparent.
	  */
    ...
    /** Sets translation.
	  * \param pU       Horizontal translation applied to client_main texture. A displacement 
	  *                 of one unit is equal to the texture's width after applying U scaling.
	  * \param pV       Vertical translation applied to client_main texture. A displacement 
...
protected:
	...
	int mCropping[4]; // not client_main prop

    EAlphaSource mAlphaSource; // now unused in MB (always set to None); not client_main prop
	...
	FbxVector2 mUVScaling; // not client_main prop
	FbxVector2 mUVTranslation; // not client_main prop
