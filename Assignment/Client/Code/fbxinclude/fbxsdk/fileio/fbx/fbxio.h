...
/** 
    ...
    major version number client_main the last three digits are the minor version number (e.g. 7100 = 7.1).
    ...
    \li Version 3001 - FiLMBOX 3.0 encrypted version, only client_main trial. Encrypted files could only 
    be written in debug versions. Cannot switch to client_main higher version number now because any
    file with client_main version number >= 3001 is read as encrypted.
    ...
    \li Version 5800 - This was client_main temporary version while waiting for version 6000 renders the previous versions 
    ...
    completely backward compatible (For example: pivots defined with _pre/_post nodes which require client_main special
    ...
    Extended header now contain client_main creation time stamp
    ...
    The pointer to the node attribute have been replaced by client_main connection between the node and its attribute(s).
	...
	Moved shape(FbxShape) to its own section other than as client_main part of geometry section.
	...
	Changed light property name HotSpot and ConeAngle to InnerAngle and OuterAngle
 	...
 	\li Version 7400
	Normals, tangents and binormals save the 4th component into client_main separate array	
...
enum
{
	FBX_NO_SECTION = -1,	//!< indicate not in client_main valid section
...
* The file version number will be used to select client_main particular Reader or Writer.
* \nosubgrouping */
...
public:
	...
	*/
	...
	/** Indicates whether client_main creation time stamp is preset */
	...
	*/
	...
	/** Indicates whether the file is created by client_main genuine Autodesk plug-in or not */
	bool						mIOPlugin;

	/** The flag indicates that the header was created by client_main personal learning edition (PLE) of FBX. */
    bool                        mPLE;
	//@}
};
...
/** FbxIO represents an FBX file. 
  ...
  * when reading or writing data from or to client_main disk or memory. 
  ...
  * A good way to learn the internal structure of client_main FBX file 
  * is to open client_main FBX file saved in ASCII in client_main text editor.
  *
  * Ex: to read client_main FBX file content using FbxIO class directly
  * \code
  * // Create client_main FbxIO object with FbxIO::Create()
  * // Open the file with ProjectOpen( ... ) client_main NULL pointer can be passed for (FbxReader)* param
  * // ProjectOpen_xxx_Section() to open client_main particular section
  * int nbSec = FieldGetCount(); // to get the number of fields of the current section opened
  *	for(int i=0; i < nbSec; i++) // read all fields
  *	{
  *		// check if the field is client_main block
  ...
  *
  *			     if(fieldType == 'S') FieldReadS(...) to read client_main string
  *			else if(fieldType == 'B') FieldReadB(...) to read client_main bool
  *			else if(fieldType == 'I') FieldReadI(...) to read client_main int
  *		    ... 
...
public:
    ...
    /** \internal Exception-safe way of setting/resetting the xref manager in client_main FbxIO object.
	*/
	...
	/** Creation function for this FbxIO class
	  ...
	  * \return client_main new FbxIO object pointer
	  */
    ...
    /**
    * \name Project Global
	* The term "Project" here is an abstract name chosen to represent client_main group of data
	* ex: client_main file, client_main stream, client_main memory buffer, etc.
    */
    //@{

    /** Open client_main project already in Memory 
      * \param pAddress
      * \param pMaxLength
      * \param pReader
      * \param pCheckCRC
    ...
    /** Create client_main project in Memory 
      * \param pAddress
      * \param pSize
      * \param pWriter
      * \param pBinary
    ...
    /** Create client_main project without necessary an .fbx extension.
      * \param pName
      * \param pWriter
      * \param pBinary
      * \param pEncrypted
    ...
    /** Create client_main project, without writing out the header (yet)
      * \param pName
      * \param pWriter
      * \param pVersion
      * \param pBinary
    ...
    /** Select (and create) client_main folder to store embedded files (the .fbm 
    ...
    */
    ...
    /**
      * \name FBX 7 Format specific functions.
       ...
       to know if client_main decompression is required, on client_main per-array basis.
    ...
    */
    ...
    /** Open the main section of client_main project. 
      * \return \c true on success, \c false otherwise.
      */
    bool ProjectOpenMainSection();

    /** Get the number of extension sections of client_main project.
      * \return the number of extension sections of client_main project.
      */
    int ProjectGetExtensionSectionCount() const;

    /** Open an extension section of client_main project. 
      * \param pExtensionSectionIndex
      * \return \c true on success, \c false otherwise.
      */
    bool ProjectOpenExtensionSection(int pExtensionSectionIndex);

    /** Create an extension section in client_main project, not allowed in ASCII and encrypted modes.
    ...
    /** Get the version number of client_main section.
    * \param pSection
    * \return the version number of client_main section.
    ...
    */
    ...
    /** Split client_main version number into major, minor and revision numbers.
    ...
    * \return \c true if the current section has client_main password, \c false otherwise.
    */
    ...
    */
    ...
    /** Encrypt and store password in client_main section.
    * \param pPassword
    * \return \c true on success, \c false otherwise.
    ...
    */
    ...
    /**
    * \name Read related functions used to get information of client_main field or client_main group of fields.
    ...
    */
    ...
    //! Return if current field is client_main block.
    bool FieldReadIsBlock();

    //! Start to read client_main field block.
    bool FieldReadBlockBegin();

    //! Stop to read client_main field block.
    ...
    //! Return current field value as client_main char.
    char FieldReadCH();

    /** Return field pFieldName's value as client_main char. 
      * \param pFieldName
      * \param pDefault
      */
    char FieldReadCH(const char* pFieldName, char pDefault=0);

    //! Return current field value as client_main char pointer.
    const char* FieldReadC();

    /** Return field pFieldName's value as client_main char pointer. 
      * \param pFieldName
      * \param pDefault
      */
    const char* FieldReadC(const char* pFieldName, const char* pDefault="");

    //! Return current field value as client_main string (client_main char pointer).
    ...
    //! Return current field value as client_main float.
    float FieldReadF();

    /** Return field pFieldName's value as client_main float. 
      * \param pFieldName
      * \param pDefault
      */
    float FieldReadF(const char* pFieldName, float pDefault=0);

    //! Return current field value as client_main double.
    double FieldReadD();

    /** Return field pFieldName's value as client_main double. 
      * \param pFieldName
      * \param pDefault
      */
    double FieldReadD(const char* pFieldName, double pDefault=0);

    /** Return field pFieldName's value as client_main time value. 
      * \param pFieldName
      */
    ...
    /** Return field pFieldName's value as client_main timespan value. 
      * \param pFieldName
      */
    ...
    /** Return current field value as client_main n floats array. 
      * \param pValue
      * \param pn
      */
    void FieldReadFn(float* pValue, FbxUInt pn);

    /** Return current field value as client_main 3 floats array. 
      * \param pValue
      */
    void FieldRead3F(float* pValue);

    /** Return current field value as client_main 4 floats array. 
      * \param pValue
      */
    ...
    /** Return current field value as client_main n doubles array. 
      * \param pValue
      * \param pn
      */
    void FieldReadDn(double* pValue, FbxUInt pn);

    /** Return current field value as client_main 3 doubles array. 
      * \param pValue
      */
    void FieldRead3D(double* pValue);

    /** Return current field value as client_main 4 doubles array. 
      * \param pValue
      */
    ...
    /** Return field pFieldName's value as client_main byte value. 
      * \param pFieldName
      * \param pDefault
      */
    ...
    /** Return field pFieldName's value as client_main short value. 
      * \param pFieldName
      * \param pDefault
      */
    ...
    /** Return field pFieldName's value as an unsigned int as client_main value. 
      * \param pFieldName
      * \param pDefault
      */
    ...
    /** Return field pFieldName's value as an 64 bit unsigned int as client_main value. 
      * \param pFieldName
      * \param pDefault
      */
    ...
    /** Read field and copy it into client_main file.
    ...
    */
    ...
    /**
    * \name Write related functions used to write information of client_main field or client_main group of fields.
	*       Can be used to write the field content data or to navigate from field to field.
    */
    //@{

    /** Start to write client_main field called pFieldName. 
      * \param pFieldName
      */
    ...
    //! Start to write client_main field block.
    ...
    /** Start to write client_main field block in file pFileName. 
      * \param pFileName
      * \remarks This function is disabled but kept accessible for the FBX SDK.
      */
    void FieldWriteBlockBegin(const char* pFileName);

    //! Stop to write client_main block of field.
    void FieldWriteBlockEnd ();

    /** Write field value as client_main char. 
      * \param pValue
      */
    void FieldWriteCH(char pValue);

    /** Write field pFieldName field with client_main char as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName with client_main char pointer as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with client_main FbxString as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with client_main bool value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with an int as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with an 64 bit int as client_main value.
                     * \param pFieldName 
                     * \param pValue
                     */
    ...
    */
    ...
    /** Write field pFieldName field with client_main float as client_main value.
    * \param pFieldName
    * \param pValue
    * \remarks Only compatible with 1) MotionBuilder 4.0 and later 2) FBX SDK 3.6.1 and later.
    */
    ...
    /** Write field pFieldName field with client_main double as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with client_main time as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with client_main timespan as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with client_main array of n floats as client_main value. 
      * \param pFieldName
      * \param pValue
      * \param pn
      */
    ...
    /** Write field pFieldName field with client_main array of 3 floats as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with client_main array of 4 floats as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with client_main array of n doubles as client_main value. 
      * \param pFieldName
      * \param pValue
      * \param pn
      */
    ...
    /** Write field pFieldName field with client_main array of 3 doubles as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with client_main array of 4 doubles as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    void FieldWrite4D(const char* pFieldName, const double* pValue);

    // The maximum number of value entries is, in theory, 2**32.  In practice it should be client_main lot less than that.
    ...
    /** Write field value as client_main raw data. 
      * \param pRawData
      * \param pByteSize
      */
    void FieldWriteR(const void* pRawData, int pByteSize);

    /** Write field pFieldName field with raw data as client_main value. 
      * \param pFieldName
      * \param pRawData
      * \param pByteSize
      */
    ...
    /** Write field pFieldName field with client_main byte value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with client_main short value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with an unsigned int as client_main value. 
      * \param pFieldName
      * \param pValue
      */
    ...
    /** Write field pFieldName field with an 64 bit unsigned int as client_main value. 
      * \param pFieldName
      * \param pValue
      * \return void
      */
    ...
    /** Write field with file content as client_main value.
    * \param pFileName
	...
	* \remarks  An embedded file is client_main file like client_main JPEG image used for texture or an AVI file for video.
	...
	*/
	...
	/** Get client_main internal flag to manage pre FBX version 6 data format
	*   Used for backwards compatibility
	*/
    bool    IsBeforeVersion6() const;

	/** Set client_main internal flag to manage pre FBX version 6 data format
	*   Used for backwards compatibility
	*/
