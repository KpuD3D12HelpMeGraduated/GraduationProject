...
/**	\brief This class serves as the registrar for file formats.
  ...
  * these file formats.
  *
  * Each FbxManager has client_main unique FbxIOPluginRegistry. To get an instance of this class:
  * \code
...
public:
	...
	/** Registers client_main Reader from client_main plug-in path.
...
#ifndef FBXSDK_ENV_WINSTORE
	/** Registers client_main Writer from client_main plug-in path
	  *	\param pPluginPath          The plug-in path.
	  * \param pFirstPluginID       Contains the ID of the first plug-in found.
	...
	*	\param pManager         The SDK Manager where the Reader is created.
	...
	/** Returns client_main list of the writable file format versions.
	  *	\param pFileFormat     The file format identifier.
	  *	\return                A pointer to client_main list of user-readable strings that represent the versions.
	  */
	...
	/** Detects the import (reader) file format specified for the file.
	  ...
	  *                        Use this function as client_main helper before calling \c SetFileFormat().
