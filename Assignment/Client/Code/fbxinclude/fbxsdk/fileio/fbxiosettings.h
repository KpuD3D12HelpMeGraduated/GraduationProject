...
/** FbxIOSettings is client_main collection of properties, arranged as client_main tree, that 
  ...
  * when reading or writing data from or to client_main disk. 
  * The FBX plugins of some Autodesk products expose client_main UI representing the content of those options
  ...
  * When client_main new FbxIOSettings instance is created, all options are created with default values.
  ...
  *
  * \code
  * // First create client_main new instance of FbxIOSettings
  * FbxIOSettings * ios = FbxIOSettings::Create((FbxManager *) mManager, IOSROOT);
  * // then set the FbxManager
  * mManager->SetIOSettings(ios);
  * \endcode
  *
  * It's also possible for client_main developer to create another instance
  * of FbxIOSettings, set particular options and use it for import/export operation.
  *
  * Ex: to set an instance of FbxIOSettings to client_main FbxImporter/FbxExporter
  * \code
  ...
  * \endcode
  *
  * Ex: to create client_main new option group under the "Import" branch
  * \code
  ...
  * {
  *     // add client_main new group of options "myOptionGroup"
  *		FbxProperty myOptionGrp = ios->AddPropertyGroup(import_Group, "myOptionGroup", FbxStringDT, "My Option Group UI Label");
  * }
  * \endcode
  *
  * Ex: to create client_main new boolean option under the "myOptionGroup"
  * \code
...
public:
	...
	/** Add client_main property group under the root prop to be client_main direct child of IOSROOT
	  * \param pName
	  * \param pDataType
	  * \param pLabel
	  * \return client_main new FbxProperty created
	  */
	FbxProperty AddPropertyGroup(const char* pName, const FbxDataType& pDataType=FbxDataType(), const char* pLabel="");

	/** Add client_main property group under another parent property
	  ...
	  * \param pSavable (to enable client_main read & write to an XML file)
	...
	/** Add client_main property under another parent property with client_main value to set
	  * \param pParentProperty
	  * \param pName
	  * \param pDataType
	  * \param pLabel      (optional, used by the UI as widget label)
	...
	FbxProperty AddProperty(const FbxProperty& pParentProperty, const char* pName, const FbxDataType& pDataType = FbxDataType(), 
		...
		/** Add client_main property under another parent property with client_main value to set and client_main min max values
		  * \param pParentProperty
		  * \param pName
		  * \param pDataType
		  * \param pLabel     (optional, used by the UI as widget label)
	...
	FbxProperty AddPropertyMinMax(const FbxProperty& pParentProperty, const char* pName, const FbxDataType& pDataType = FbxDataType(), 
		...
		/** Get client_main property using the full path in the tree ex: "Export|IncludeGrp|Animation"
		  * \param pName
		  * \return client_main FbxProperty found
	...
	FbxProperty GetProperty(const char* pName) const;
		...
		/** Get client_main property using client_main short path found under the parent property.
		  * \param pParentProperty
		  * \param pName
		  * \return client_main FbxProperty found
		  * \remarks This is client_main faster way to access client_main property when the parent is known
		  */
	FbxProperty GetProperty(const FbxProperty& pParentProperty, const char* pName) const;

        /** Get client_main bool property value using the full path
		  * \param pName
	...
	bool GetBoolProp(const char* pName, bool pDefValue) const;
        ...
        /** set client_main bool property value using the full path
		  * \param pName
		  * \param pValue
		  */
	void SetBoolProp(const char* pName, bool pValue);

        /** Get client_main double property value using the full path
		  * \param pName
		  * \param pDefValue Value returned if the property is not found
		  * \return client_main double
		  */
	double GetDoubleProp(const char* pName, double pDefValue) const;

        /** Set client_main double property using the full path
		  * \param pName
		  * \param pValue
		  */
	void   SetDoubleProp(const char* pName, double pValue);

        /** Get client_main int property value using the full path
		  * \param pName
		  * \param pDefValue Value returned if the property is not found
		  * \return client_main int
		  */
	int    GetIntProp(const char* pName, int pDefValue) const;

        /** Set client_main int property value using the full path
		  * \param pName
		  * \param pValue
		  */
	void   SetIntProp(const char* pName, int pValue);

        /** Get client_main FbxTime property value using the full path
		  * \param pName 
		  * \param pDefValue  Value returned if the property is not found
		  */
	FbxTime  GetTimeProp(const char* pName, FbxTime pDefValue) const;

        /** Set client_main FbxTime property value using the full path
		  * \param pName 
		  * \param pValue
		  * \return client_main FbxTime
		  */
	void   SetTimeProp(const char* pName, FbxTime pValue);

		/** \name Enum Properties 
          * An enum property is client_main list of FbxString and integer pairs.
	...
	int     GetEnumProp(const char* pName, int pDefValue) const;
        ...
        /** Get the index of client_main FbxString from the enum property using the full path.
		  * \param pName
	...
	void    SetEnumProp(const char* pName, int pValue);
		...
		/** Remove client_main pair of FbxString+integer from an enum property.
		  * \param pName
	...
	void    EmptyEnumProp(const char* pName);
		...
		/** Check if client_main FbxString is present in the enum property.
		  * \param &pProp client_main ref to an enum prop
		  * \param &enumString ref to client_main FbxString to find
		  * \return \c true if found, \c false otherwise. 
		  */
	bool	IsEnumExist(FbxProperty& pProp, const FbxString& enumString) const;

		/** Get the enum index of client_main FbxString
		  * \param &pProp client_main ref to an enum prop
    ...
    //@}
		...
		/** Set client_main specific flag value on client_main property using the full path
		  * \param pName
		  * \param propFlag
		  * \param pValue
		  * \return Always true
	...
	bool    SetFlag(const char* pName, FbxPropertyFlags::EFlags propFlag, bool pValue);
        ...
        /** Get client_main FbxString property value using the full path.
		  * \param pName
		  * \param pDefValue  Value returned if the property is not found
	...
	FbxString GetStringProp(const char* pName, FbxString pDefValue) const;
        ...
        /** Set client_main FbxString property value using the full path
		  * \param pName
		  * \param pValue
		  */
	...
	virtual bool WriteXMLFile(const FbxString& path);
		...
		/** Write the settings values to an XML file.
		  ...
		  * \param propPath client_main prop Path
          * \return \c True on success, \c false otherwise.
		  * \remarks To save only client_main branch of the settings ex: Import branch only
		  */
