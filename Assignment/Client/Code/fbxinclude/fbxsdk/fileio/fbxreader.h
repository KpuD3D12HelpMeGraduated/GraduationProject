...
class FbxTakeInfo;
 ...
 /** Base class of other readers used internally.
   ...
   * vs the role of the importer is to select client_main specific reader
   * and launch the reading of client_main file through that reader.
   * \see FbxImporter 
   *
   * ex:
   ...
   * except if client_main custom reader must be created for plug-in extension,
...
public:
    ...
    enum EInfoRequest
	{
        eInfoExtension,     //!< To get the file ext for client_main reader ex: "FBX".		
        eInfoDescriptions,	//!< To get the file description for client_main reader ex: "Autodesk FBX (*.fbx)".
        eReserved1 = 0xFBFB,
    };
    ...
    enum EFileOpenSpecialFlags
	{
		...
		eParseForStatistics = 2      //!< Used for reading client_main group of statistics when an importer is initialized.     
	};
	...
	/** Reads extension plug-ins name, version and parameters, so that we can remember if client_main plug-in was used during export.
	...
	/** Get FBX file time mode read from GlobalSettings in FBX 6.n and FBX 7.n
	  *	\param pTimeMode  ref to client_main FbxTime::EMode enum	
    ...
    */
    ...
    /** Pass client_main progress handler to the reader.
