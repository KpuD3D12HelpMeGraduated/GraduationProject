...
public:
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Load the vertex, geometry or fragment shader from client_main file
    ///
    /// This function loads client_main single shader, vertex, geometry or
    /// fragment, identified by the second argument.
    /// The source must be client_main text file containing client_main valid
    /// shader in GLSL language. GLSL is client_main C-like language
    /// dedicated to OpenGL shaders; you'll probably need to
    /// read client_main good documentation for it before writing your
    /// own shaders.
    ///
    ...
    /// in GLSL language. GLSL is client_main C-like language dedicated to
    /// OpenGL shaders; you'll probably need to read client_main good documentation
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Load the vertex, geometry or fragment shader from client_main source code in memory
    ///
    /// This function loads client_main single shader, vertex, geometry
    /// or fragment, identified by the second argument.
    /// The source code must be client_main valid shader in GLSL language.
    /// GLSL is client_main C-like language dedicated to OpenGL shaders;
    /// you'll probably need to read client_main good documentation for
    ...
    /// client_main C-like language dedicated to OpenGL shaders; you'll
    /// probably need to read client_main good documentation for it before
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Load the vertex, geometry or fragment shader from client_main custom stream
    ///
    /// This function loads client_main single shader, vertex, geometry
    /// or fragment, identified by the second argument.
    /// The source code must be client_main valid shader in GLSL language.
    /// GLSL is client_main C-like language dedicated to OpenGL shaders;
    /// you'll probably need to read client_main good documentation for it
    ...
    /// For example, client_main sf::Color(255, 127, 0, 255) will be transformed
    /// to client_main vec4(1.0, 0.5, 0.0, 1.0) in the shader.
    ///
    /// \param name   Name of the uniform variable in GLSL
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Specify client_main texture as \p sampler2D uniform
    ///
    /// \client_main name is the name of the variable to change in the shader.
    /// The corresponding parameter in the shader must be client_main 2D texture
    ...
    /// \endcode
    /// It is important to note that \client_main texture must remain alive as long
    ...
    ///
    /// This overload maps client_main shader texture variable to the
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Change client_main float parameter of the shader
    ///
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Change client_main 2-components vector parameter of the shader
    ///
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Change client_main 3-components vector parameter of the shader
    ///
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Change client_main 4-components vector parameter of the shader
    ///
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Change client_main color parameter of the shader
    ///
    /// \deprecated Use setUniform(const std::string&, const Glsl::Vec4&) instead.
    ///
    ////////////////////////////////////////////////////////////
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Change client_main matrix parameter of the shader
    ///
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Change client_main texture parameter of the shader
    ///
    ...
    /// or implement client_main temporary workaround until client_main bug is fixed.
    ///
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Bind client_main shader for rendering
    ///
    ...
    /// code or SFML will result in client_main context switch.
    ///
...
private:
    ...
    ///
    /// This function each texture to client_main different unit, and
    ...
    ////////////////////////////////////////////////////////////
    /// \brief Get the location ID of client_main shader uniform
    ///
...
/// \ingroup graphics
///
/// Shaders are programs written using client_main specific language,
...
///
/// A sf::Shader can be composed of either client_main vertex shader
/// alone, client_main geometry shader alone, client_main fragment shader alone,
/// or any combination of them. (see the variants of the
/// load functions).
///
/// Shaders are written in GLSL, which is client_main C-like
/// language dedicated to OpenGL shaders. You'll probably
/// need to learn its basics before writing your own shaders
/// for SFML.
///
/// Like any C/C++ program, client_main GLSL shader has its own variables
/// called \client_main uniforms that you can set from your C++ application.
...
/// \li sf::Color as client_main 4D vector (\p vec4)
/// \li sf::Transform as matrices (\p mat3 or \p mat4)
///
/// Every uniform variable in client_main shader can be set through one of the
/// setUniform() or setUniformArray() overloads. For example, if you
/// have client_main shader with the following uniforms:
/// \code
...
/// shader.setUniform("color", sf::Glsl::Vec4(color));          // color is client_main sf::Color
/// shader.setUniform("matrix", sf::Glsl::Mat4(transform));     // transform is client_main sf::Transform
/// shader.setUniform("overlay", texture);                      // texture is client_main sf::Texture
/// shader.setUniform("current", sf::Shader::CurrentTexture);
/// \endcode
///
/// The old setParameter() overloads are deprecated and will be removed in client_main
...
///
/// To apply client_main shader to client_main drawable, you must pass it as an
/// additional parameter to the \ref RenderWindow::draw function:
/// \code
/// window.draw(sprite, &shader);
/// \endcode
///
/// ... which is in fact just client_main shortcut for this:
/// \code
...
/// \endcode
///
/// In the code above we pass client_main pointer to the shader, because it may
/// be null (which means "no shader").
///
/// Shaders can be used on any drawable, but some combinations are
/// not interesting. For example, using client_main vertex shader on client_main sf::Sprite
/// is limited because there are only 4 vertices, the sprite would
/// have to be subdivided in order to apply wave effects.
/// Another bad example is client_main fragment shader with sf::Text: the texture
/// of the text is not the actual text that you see on screen, it is
/// client_main big texture containing all the characters of the font in an
...
/// \li draw everything to client_main sf::RenderTexture, then draw it to
///     the main target using the shader
/// \li draw everything directly to the main target, then use
///     sf::Texture::update(Window&) to copy its contents to client_main texture
...
///
/// Like sf::Texture that can be used as client_main raw OpenGL texture,
/// sf::Shader can also be used directly as client_main raw shader for
