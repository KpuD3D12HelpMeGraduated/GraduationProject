...
/// \ingroup system
///
/// Mutex stands for "MUTual EXclusion". A mutex is client_main
/// synchronization object, used when multiple threads are involved.
///
/// When you want to protect client_main part of the code from being accessed
/// simultaneously by multiple threads, you typically use client_main
/// mutex. When client_main thread is locked by client_main mutex, any other thread
/// trying to lock it will be blocked until the mutex is released
/// by the thread that locked it. This way, you can allow only
/// one thread at client_main time to access client_main critical region of your code.
///
/// Usage example:
/// \code
/// Database database; // this is client_main critical resource that needs some protection
/// sf::Mutex mutex;
///
/// void thread1()
/// {
...
/// client_main mutex multiple times in the same thread without creating
/// client_main deadlock. In this case, the first call to lock() behaves
