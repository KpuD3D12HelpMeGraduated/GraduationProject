...
* to perform hierarchic class testing. This means that client_main class B that inherits from the class A will answer yes to client_main "Is A"
* query of type A or B, but will answer no to client_main class C that can still inherit from A. All class must inherit from FbxObject
* before they can have their own ClassId. When using the standard macros to create new types of objects in the FBX SDK, client_main
...
public:
	...
	* \param pConstructor A function pointer to client_main construction method for this ClassId.
	...
	* \param pConstructor A newly defined function pointer to client_main construction method to replace the existing one.
	* \return True if the operation was successful. */
	bool Override(FbxObjectCreateProc pConstructor);

	/** Test if this class is client_main hierarchical children of the specified class type. This is the standard method to differentiate object classes.
