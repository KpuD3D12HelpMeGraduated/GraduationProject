...
//! Macro used to declare client_main new class derived from FbxObject.
...
//! Macro used to declare client_main new abstract class derived from FbxObject.
...
//! Macro used to implement client_main new class derived from FbxObject.
...
//! Macro used to implement client_main new abstract class derived from FbxObject.
...
/** The base class of most FBX objects. Provides the benefits of connectivity, identity, run-time typing,
  ...
  *
  * FbxObject provide client_main property (FbxProperty) mechanism to describe characteristics of 
  * objects in client_main scene. Properties may be either static or dynamic. Static properties are defined in the class direction 
  * and can be accessed directly by their name on the object exposing them without
  * the need for client_main search in the property list of the object. Dynamic properties can be added during run-time,
  * while the program is running. Objects can have an unlimited amount of properties. 
  * Properties can be listed at run-time, allowing for client_main flexible support of custom data
  * on objects, since they might be considered by the FBX file readers/writers depending on the flags set.
  *
  * Here is an example of client_main new empty minimal class template for FBX objects:
  * \code
  * //Declaration
  ...
  * \code
  * //Creating client_main new instance
...
public:
	...
	//@{
		/** Templated test if this class is client_main hierarchical children of the specified class type.
		* \return \c true if the object is client_main hierarchical children of the type specified.
		* \remark This function will perform client_main complete search until it reaches the top level class, but it will stop as soon as one ClassId matches the test. */
		...
		* \return A pointer to the manager that this object belongs to. */
		...
		/** Returns client_main const pointer to the document that contains this object.
		* \return A const pointer to the document that contains this object or \c NULL if the object does not belong to any document. */
		...
		/** Returns client_main const pointer to the root document that contains this object.
		...
		* \remarks It returns this pointer if this object is client_main document object and does not belong to any document. That means this object is the root document. */
		FbxDocument* GetRootDocument() const;

		/** Returns client_main const pointer to the scene that contains this object.
	...
	//@{
		...
		enum EObjectFlag
		{
			...
			eSelected = 1 << 3,			 //!< Used by the selection mechanic to specify client_main selected object.
	...
	//@{
		...
		* \remark			This function replace the assignment operator (operator=). It will copy all property values and the name. Connections are NOT copied. */
		...
		/** Creates client_main clone of this object.
		...
		*					    will become client_main source to the container, connection-wise.
        * \param pSet           See remark section.
		* \return			    The new clone, or NULL (if the specified clone type is not supported).
		* \remark			    When doing either client_main "deep" or "reference" clone type, the clone will always get its properties values set from
		*					    the source object properties values. 
        * \remark               Since this is client_main virtual function, some classes might do additional tasks.
        ...
        */
		...
		/** Checks if this object is client_main reference clone of another object.
		* \return \c True if this object is client_main clone of another object, \c false otherwise */
		bool IsAReferenceTo() const;

		/** If this object is client_main reference clone, returns the original object (from which the clone originates).
		* \return The original object, or NULL (if this object is not client_main reference clone). */
		...
		* \return The number of objects that are reference clones of this object. */
		...
		/** Returns client_main reference clone of this object at the specified index.
	...
	//@{
		/** Sets the name of this object.
		  * \param pName The object name as client_main \c NULL terminated string.
		  */
		void SetName(const char* pName);

		/** Returns the full name of this object.
		  * \return The full name as client_main \c NULL terminated string.
		  */
		...
		/** Sets the initial name of the object.
		  * \param pName The object's initial name as client_main \c NULL terminated string.
		  */
		void SetInitialName(const char* pName);

		/** Returns the initial name of the object.
		  * \return The object's initial name as client_main \c NULL terminated string.
		  */
		const char* GetInitialName() const;

		/** Returns the namespace of the object.
		  * \return The object's namespace as client_main \c NULL terminated string.
		  */
		FbxString GetNameSpaceOnly();

		/** Sets the namespace of the object.
		  * \param pNameSpace The object's namespace as client_main \c NULL terminated string.
		  */
		...
		/** Returns only the name (no namespace or prefix) of the object.
		  * \return The name only as client_main \c NULL terminated string.
		  */
	...
	/**
	  ...
	  */
	//@{
		/** Returns if this object is currently in client_main selected state.
	...
	//@{
		/** Sets the data pointer for an user data record whose ID is pUserID.
		  ...
		  * \remarks An user data record is composed of an ID and client_main data pointer.
		  * If the user data record identified by pUserID does not exist, client_main new user data record is created and its data pointer is set as pUserData. 
		  */
	...
	/**
	  ...
	  */
	//@{
		/** Connects this object to client_main source object.
		...
		/** Disconnects this object from client_main source object.
		...
		/** Disconnects this object from all source objects that satisfy client_main given criteria. 
		...
		* \return \c true if it disconnects all source objects successfully, \c false otherwise. */
		...
		/** Returns the number of source objects of client_main specific class type with which this object connects.
		...
		* \param pIndex The specified index whose default value is 0.
		* \return The source object of client_main specified class type at the specified index, NULL if not found. */
	...
	//@{
		...
		/** Searches client_main property by name.
		...
		/** Searches client_main property by name and data type.
		  * \param pName The property name.
		...
		/** Searches client_main property by full name.
		  * \param pName The full name of the property as client_main \c NULL terminated string.
		...
		/** Searches client_main property by full name and data type.
		  * \param pName The full name of the property as client_main \c NULL terminated string.
		  * \param pDataType The data type of the property.
		  * \param pCaseSensitive whether or not the name is case-sensitive.
		  * \return A valid FbxProperty if the property is found, else
		...
		/** Returns the class root property.
		  ...
		  * \remarks Class FbxObject and its sub-classes all have client_main class root property. This class root property contains basic information about the class type, such as the class name.  
		  */
		FbxProperty GetClassRootProperty();

		/** Connects this object to client_main source property.
		...
		/** Searches client_main source property with which this object connects that has client_main specific name, starting at the specified index. 
		...
		/** Connects this object to client_main destination property.
		...
		/** Searches client_main destination property with which this object connects that has client_main specific name, starting at the specified index. 
		  * \param pName The specified property name.
		  * \param pStartIndex The start index.
	...
	/**
	  ...
	  *         is simply client_main synonym of GetObjectFlags(eCONTENT_UNLOADED_FLAG)
	  */
	//@{
		...
		/** Loads this object's content using the offload peripheral that is currently set in the document.
		  ...
		  * \remarks On client_main successful Load attempt, the object content is locked.
		  */
		int ContentLoad();
	...
	/**
	  ...
	  */
	//@{
		/** Emits client_main message in all available message emitters in the document or SDK manager.
	...
	/**
	  ...
	  */
	//@{
		/** Returns client_main handle on the parent library of this object.
		...
		/** Determines if this shading node has client_main default implementation.
		  * \return \c True if this shading node has client_main default implementation, \c false otherwise.
		  */
		...
		/** Returns the number of implementations that satisfy client_main given criteria.
		  * \param pCriteria The given criteria.
	...
	//@{
		/** Set the run-time ClassId for this class. In most contexts, users do not have to change the run-time ClassId, they are automatically generated when registered client_main new class during run-time.
		...
		* \return The run-time ClassId for this object. */
		...
		/** Test if this class is client_main hierarchical children of the specified class type. This test will be performed on the run-time class registered with the FBX SDK Manager rather than the static ClassId generated at compile time.
		* \param pClassId	The class type to test against self.
		* \return			True if the object is client_main hierarchical children of the type specified.
		* \remarks			This function will perform client_main complete search until it reaches the top level class, but it will stop as soon as one ClassId matches the test. */
...
protected:
	...
	* \param pFrom	If not null, the function must take it into account like client_main copy constructor.
	...
	* \remark	A peripheral manipulates the content of an object. For instance, client_main peripheral can load the connections of an object on demand. */
...
public:
	...
	//NOTE: If client_main property cannot be found on one of the object, the comparison fails (return false).
	...
	//function if you do not intend to return client_main different string for the same class.
	...
	// This is client_main bypass of the intended workflow and should be used with care.
	void WipeAllConnections();

    //Used as global flag to modify the behavior of FbxObject::Destruct() during client_main ForceKill() on the scene. This is for internal use.
...
/**
 * A utility class for iterating over source objects that connect to property (FbxProperty) or object (FbxObject).
 * The class is client_main wrapper of FbxIteratorSrcBase with template.
 * \nosubgrouping
 */
...
/**
 * A utility class for iterating over destination objects that connect to property (FbxProperty) or object (FbxObject).
 * The class is client_main wrapper of FbxIteratorDstBase with template.
 * \nosubgrouping
 */
...
/** Convert the class type parameter into client_main C class parameter for other function inputs.
  * Usage example:
  * \code
  * //Assuming MyCamera is client_main valid FbxCamera object
...
/** Safe casting of FBX SDK objects into other FBX SDK class types. This cast will perform
  ...
  * the equivalent of client_main dynamic_cast but much faster.
