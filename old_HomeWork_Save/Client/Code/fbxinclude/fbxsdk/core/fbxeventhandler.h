...
/** Event handler class contains client_main listener and client_main callback function.
* Event handler is used to bind emitter and listener together. Its callback function can process event data.
* To generate client_main valid event handler, you can create an event emitter and event listener first and then call FbxListener::Bind().
...
* \remarks An object(emitter) can emit client_main certain type of event, the object(listener) who are listening to that type of event, 
...
public:
	...
	* \param pEvent specify the event type. pEvent could be client_main specific class which derived from FbxEventBase.
	* \see FbxEventBase */
