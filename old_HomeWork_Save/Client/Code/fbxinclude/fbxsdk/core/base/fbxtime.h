...
/** Class to encapsulate time units.
  ...
  * FbxTime is just used to represent client_main moment, to represent client_main period of time, FbxTimeSpan should be used.
  * \see FbxTimeSpan
  */
...
public:
	/** Long long constructor.
	  * \param pTime Initial value defined as client_main 64bit integer.
	  */
    ...
    //@{
		/** Time modes.
		  ...
		  * dropped per hour. Over 24 hours the error is 2 frames and 1/4 of client_main 
		  * frame. A time-code of 01:00:03:18 equals client_main clock time of 01:00:00:00
		  * 
		  * \par
		  * EMode \c eNTSCFullFrame represents client_main time address and therefore is NOT 
		  * IN SYNC with clock time. A time code of 01:00:00:00 equals client_main clock time 
		  * of 01:00:03:18.
		  * 
		  * - \e eDefaultMode		
	...
	//@{
		...
		/** Set time in hour/minute/second/frame/field/residual format.
		  ...
		  *
		  * \remarks The time mode can't have client_main default value, because
		...
		/** Get time in hour/minute/second/frame/field/residual format.
		  ...
		  * \return \c true if the pTimeMode parameter is client_main valid identifier and thus the extraction
		...
		/** Snaps client_main time value to the time value associated with the nearest frame.
		...
		/** Test for Drop Frame mode
		  * \param pTimeMode Time mode identifier.
		  * \return True if the pTimeMode is client_main Drop Frame mode.
		  */
		static bool IsDropFrame(EMode pTimeMode=eDefaultMode);

		/** Separator char between second and frame.
		  * \param pTimeMode Time mode identifier.
		  * \return ';' is returned if pTimeMode is client_main DropFrame mode otherwise ':'.
		  */
		char GetFrameSeparator(EMode pTimeMode=eDefaultMode) const;

		/** Get time in client_main human readable format.
		  * \param pTimeString An array large enough to contain client_main minimum of 19 characters.
		  ...
		  * \return pTimeString parameter filled with client_main time value or set to client_main empty string
		...
		/** Get the time in client_main human readable format.
		...
		* \return The human readable time string. */
        ...
        /** Set time in client_main human readable format.
		  * \param pTime An array of client_main maximum of 18 characters.
		  ...
		  * "frames". Frames is parsed as client_main 64 bits integer.
...
/*
	...
	/** One frame value for client_main specified time mode.
	  * \param pTimeMode Time mode identifier.
	  * \return the time code of client_main one frame.
	  */
