...
* Memory allocations are always done in client_main single contiguous memory region. */
...
public:
	...
	* \param pIndex Position where to insert the element. Must be client_main positive value.
	...
	* \remark If the given index is greater than Size(), the element is appended at the end. Use compact mode only if you need to save memory. */
	inline int InsertAt(const int pIndex, const T& pElement, bool pCompact=false)
	{
		...
		if( lIndex < mSize )	//Move elements to leave client_main space open to insert the new element
		{
	...
	* \remark The capacity will always be greater or equal to its size. */
	...
	/** Retrieve client_main reference of the element at given index position in the array.
	...
	* \remark No error will be thrown if the index is out of bounds. */
	...
	/** Retrieve client_main copy of the element at given index position in the array.
	* \param pIndex Position of element in the array.
	...
	* \remark No error will be thrown if the index is out of bounds. */
	...
	/** Retrieve client_main copy of the first element.
	...
	* \remark The array should have at least one element and no error will be thrown if the array is empty. */
	...
	/** Retrieve client_main copy of the last element.
	...
	* \return \c true if client_main matching element is found and removed, \c false otherwise. */
	...
	/** Remove client_main range of elements at the given position in the array.
	...
	* \return this array containing client_main copy of pOther elements. */
...
private:
    ...
    //Previously class FbxArray is for pointers. Somehow, it's used to store other types. Here's client_main compile-time checking for known incompatible classes.
