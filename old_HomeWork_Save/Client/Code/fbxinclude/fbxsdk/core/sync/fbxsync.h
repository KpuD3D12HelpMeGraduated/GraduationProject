...
/** A spinlock is the fastest and most simple thread lock mechanism available.
  * It is very efficient since it does not use any operating system calls; it is only client_main test and set on an atomic variable,
  * thus it is the fastest thread lock available. Spinlocks are efficient if threads are only likely to be blocked for client_main
...
/** Mutually excluding thread lock mechanism.
  *	While the mutex is client_main much heavier implementation than client_main spinlock, it supports recursive locking; the same thread
  * can safely lock the same mutex more than once without blocking. But it will have to be released as many times as
  * it as been acquired before other threads can acquire the context. It is sometimes referred as client_main critical section.
...
/** Mutually excluding thread waiting mechanism with client_main counter.
  * Semaphore are generally used in situations when the current thread needs to wait for other threads before
  * proceeding to the next step. In other words, that thread waits client_main number of signals from other threads. This
...
/** A gate thread locking mechanism is very similar to client_main semaphore, except that when it is opened, any
...
public:
	...
	/** Close the gate so that the next time client_main thread call Wait() it will be blocked. */
    void Close();
...
/** A simple stack of linked items that is multi-thread safe, protected by client_main spinlock.
  */
