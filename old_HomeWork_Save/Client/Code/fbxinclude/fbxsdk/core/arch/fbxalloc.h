...
* class describes an object that stores client_main pointer to client_main single allocated object of type Type* that ensures that the object to which
* it points gets destroyed automatically when control leaves client_main scope. */
template<class Type, class Policy=FbxDeletionPolicyDefault<Type> > class FbxAutoPtr
{
public:
	//! Construct from client_main pointer.
	...
	//! Convert to client_main Type pointer.
...
/** FbxSharedPtr class describes an object that stores client_main pointer to client_main single allocated object of type 
* Type* that ensures that the object to which it points gets destroyed automatically when the control 
* leaves client_main scope and the reference count is 0. */
class RefCount
{
public:
 	RefCount()	{ Init(); };
