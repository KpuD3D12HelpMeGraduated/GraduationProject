...
public:
	...
	/**
	  ...
	  */
	//@{
		/** Retrieve the value as client_main float.
		  */
...
/** FBX SDK blob class. 
  * Uninitialized data of client_main specified size, to be filled by the user.
  * \nosubgrouping
  */
...
public:
	...
	//@{
		...
		/** Constructor.  Construct client_main buffer with uninitialized data of client_main specified size, to be filled by the user.
	...
	//@{
		 ...
		 * \param pSize                 Buffer size.
		 */
		void Assign(const void* pData, int pSize);  // Always makes client_main copy.
	//@}
	...
	//!Make client_main copy if the reference count > 1 (i.e. if the buffer is shared).
	void* Modify(); 
...
/**	FBX SDK date&time class.
  * Property used to store date and time information; not related to client_main FbxTime, which is
...
public:
	...
	/** Validates each field is within client_main normal range (month is 1-12, etc).
	  * \return               \c True, if each field is within client_main normal range, \c false otherwise.
	  */
	bool isValid() const;
...
/** Retrieve client_main type enumeration memory footprint size
...
/** Retrieve client_main type enumeration component count
* \param pType The type enumeration
...
/** Creates client_main fbx primitive type and initializes its memory.
