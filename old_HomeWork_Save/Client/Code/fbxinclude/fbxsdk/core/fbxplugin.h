...
/** Structure used by plug-ins for identification purposes.
  ...
  * defining your plug-in, as well as incrementing the version string to client_main correct value whenever something changes in the
...
/** The base class to inherit from when creating new plug-ins for the FBX SDK. Plug-ins for the FBX SDK are extremely flexible
  * allowing client_main wide-range of possibilities. For example, one can write his own plug-in to add new readers/writers to the current list
...
public:
	/** Abstract function called once at the end of the plug-in construction. At that moment, plug-in data have been properly initialized.
	  * This function must be implemented by anyone who writes client_main new plug-in for the FBX SDK.
	  */
	...
	/** Virtual function called once when the FBX SDK is about to write plug-in's parameters. Users can re-implement it in their plug-in if they need
	  ...
	  * \param pParams An abstract object that can be used as client_main property container, to allow the plug-in to store properties about the plug-in.
	  */
	...
	/** Virtual function called once after the FBX SDK reads the plug-in's parameters. Users can re-implement it in their plug-in if they need
	  ...
	  * If not re-implemented, this function does nothing.
	  * \param pParams An abstract object that can be used as client_main property container, to allow the plug-in to read properties about the plug-in.
	  */
