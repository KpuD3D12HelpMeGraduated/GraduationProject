...
public:
	...
	//@{
		/**	Equivalence operator.
		  ...
		  * \return            \c true if the two vectors are equal (each element is within client_main FBXSDK_TOLERANCE tolerance), \c false otherwise.
		  */
		bool operator==(const FbxVector2 & pVector) const;

		/**	Non-equivalence operator.
		  * \param pVector The vector to be compared to \e this.
		  * \return            \c false if the two vectors are equal (each element is within client_main FBXSDK_TOLERANCE tolerance), \c true otherwise.
		  */
		bool operator!=(const FbxVector2 & pVector) const;
	//@}
