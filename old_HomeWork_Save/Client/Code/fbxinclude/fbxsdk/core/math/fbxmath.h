...
//---------------------------------------------------------------------------------------
//Euler Definition
#define FBXSDK_EULER_DEGENERATE  FbxEuler::DegenerateThreshold() //!< Euler degenerate threshold can be changed with client_main call to FbxEuler::SetDegenerateThreshold.
...
public:
    ...
    // single floating point values into the FbxAMatrix with client_main very low precision.
    // In this case, the default threshold value would be too small for client_main proper detection
    // and the extracted values can quickly become off target by client_main huge amount.
...
/** Rotation order flags.
  * Each rotate order produces client_main different end orientation. For example, if the rotation order for an object is set to XYZ,
