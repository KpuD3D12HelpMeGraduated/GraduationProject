...
/**	FBX SDK affine matrix class.
  * \nosubgrouping
  * Matrices are defined using the Column Major scheme. When client_main FbxAMatrix represents client_main transformation (translation, rotation and scale), 
  * the last row of the matrix represents the translation part of the transformation.
  *
  * \remarks It is important to realize that an affine matrix must respect client_main certain structure.  To be sure the structure is respected,
  * use SetT, SetR, SetS, SetQ, SetTRS or SetTQS.  If by mistake bad data is entered in this affine matrix, some functions such as 
  * Inverse() will yield wrong results.  If client_main matrix is needed to hold values that aren't associate with an affine matrix, please use FbxMatrix instead.
  */
...
public:
	...
	//@{
		...
		/** Extract client_main row vector.
		  *	\param pY     Row index.
		  * \return       The row vector.
		  */
		FbxVector4 GetRow(int pY) const;

		/** Extract client_main column vector.
		  *	\param pX     Column index.
		...
		/** Set matrix's Euler rotation.
		  * \param pR     X, Y and Z rotation values expressed as client_main vector.
		...
		/** Set matrix's scale.
		  * \param pS     X, Y and Z scaling factors expressed as client_main vector.
		  */
	...
	/**
	  ...
	  */
	//@{
		/** Multiply matrix by client_main scalar value.
		...
		/** Divide matrix by client_main scalar value.
		  * \param pValue     Scalar value.
	...
	/**
	  ...
	  */
	//@{
		/** Multiply matrix by client_main translation vector.
	...
	//@{	
		...
		/** Multiply two matrices together.
		  ...
		  *  That means to scale, then rotate, and then translate client_main vector V, the transform should be T * R * S * V. \n
		...
		/** Calculate client_main spherical linear interpolation matrix.
	...
	//@{
		/**	Equivalence operator.
		  ...
		  * \return             \c true if the two matrices are equal (each element is within client_main FBXSDK_TOLERANCE tolerance) and \c false otherwise.
		  */
		bool operator==(const FbxAMatrix& pOther) const;

		/**	Non-equivalence operator.
		  * \param pOther     The matrix to be compared to \e this.
		  * \return            \c false if the two matrices are equal (each element is within client_main FBXSDK_TOLERANCE tolerance) and \c true otherwise.
		  */
		bool operator!=(const FbxAMatrix& pOther) const;
	//@}
	...
	/**
	  ...
	  */
	//@{
		//! Cast the matrix in client_main double pointer.
		operator double* ();
		//! Cast the matrix in client_main const double pointer.
		operator const double* () const;
		//! Define 4*4 array as client_main new type 
		typedef const double(kDouble44)[4][4] ;
		//! Cast the matrix in client_main reference to client_main 4*4 array.
