...
*
* An importer will select the appropriate reader to client_main particular file.
...
* then if client_main reader is found, the importer will create 
...
* -# create an importer
* -# initialize the importer with client_main file name and IOSettings
...
*
* // Initialize the importer by providing client_main filename and the IOSettings to use
...
* \endcode
*
* \remarks According to the file suffix, client_main specialized reader will be created internally.
*          Ex: for .fbx files client_main FBX Reader, for .3ds files, client_main 3ds reader, etc.
...
public:
    ...
    /** Import the currently opened file into client_main scene. 
      * \param pDocument          Document to fill with file content.
	  * \param pNonBlocking       If true, the import process will be executed in client_main new thread, allowing it to be non-blocking.
	...
	/** Set the IOSettings pointer
	  * \param pIOSettings Point to client_main FbxIOSettings object.
	  */
	...
	/** Get the take information about an available take.
	  * Use the returned reference to client_main FbxTakeInfo object to set whether the indexed take is imported.
...
//! Event that is emitted to plugins before client_main FBX file has been imported.
...
//! Event that is emitted to plugins after client_main FBX file has been imported.
