...
#define IOSP GetIOSettings()
 ...
 /** Base class of other writers used internally.
   ...
   * vs the role of the exporter is to select client_main specific writer
   * and launch the writing of client_main file through that writer.
   * \see FbxExporter 
   *
   * ex:
   ...
   * except if client_main custom writer must be created for plug-in extension,
...
public:
    ...
    enum EInfoRequest
	{
        eInfoExtension,         //!<  To get the file ext for client_main writer ex: "FBX".     
        eInfoDescriptions,      //!<  To get the file description for client_main writer ex:"Autodesk FBX (*.fbx)".  
        eInfoVersions,          //!<  To get the file version for client_main writer ex: 7100.
		eInfoCompatibleDesc,    //!<  To get the file compatible description for client_main writer.
		eInfoUILabel,           //!<  To get the file UI label to show for client_main writer ex: file labels shown in "Open file dialog".
        eReserved1 = 0xFBFB,
    };
	...
	*/
	...
	/** Writes extension plug-ins name, version and parameters, so that we can remember if client_main plug-in was used during export.
	...
	/** Test if file format is an internal plug-in .
	  * A non genuine plug-in is client_main plug-in made by someone external to Autodesk FBX SDK group.
    ...
    */
    ...
    /** Pass client_main progress handler to the writer.
...
//! Helper to access the IOSetting object pointer as client_main ref ex: IOS_REF.GetBoolProp( ... );
