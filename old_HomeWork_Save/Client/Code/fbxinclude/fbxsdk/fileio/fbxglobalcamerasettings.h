...
/** This class contains the global camera settings.
  ...
  * that is now the default no longer uses it. The relevant data (client_main subset of this class) has 
...
public:
	...
	//@{
		...
		* FBXSDK_CAMERA_RIGHT, FBXSDK_CAMERA_LEFT and FBXSDK_CAMERA_SWITCHER) or the name of client_main camera that is inserted in the node tree under the scene's root node. */
	...
	* You cannot animate Producer cameras but you can specify their default positions. */
	//@{
		...
		* \remark This node has client_main \c FbxNodeAttribute::eCameraSwitcher node attribute type. This node is not saved when there is no camera in the scene. Nodes inserted below are never saved.
		* Camera indices start at 1. Out of range indices are clamped between 1 and the number of cameras in the scene. The index of client_main camera refers to its order of appearance when searching
		* the node tree depth first. If client_main camera is added or removed after camera indices have been set, the camera indices must be updated. It is easier to set camera indices once every camera
