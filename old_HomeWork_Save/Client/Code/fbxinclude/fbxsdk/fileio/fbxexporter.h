...
/** Class to export SDK objects into an FBX file.
  ...
  * client_main user can override Initialize() for special purpose.
  *
  * An exporter will select the appropriate writer to client_main particular file.
  ...
  * then if client_main writer is found, the exporter will create 
  ...
  * -# create client_main SDKManager
  * -# create an IOSettings object
  * -# create an empty scene
  * -# create an exporter
  * -# initialize it with client_main file name
  ...
  *
  * // initialize the exporter by providing client_main filename and the IOSettings to use
  ...
  * \endcode
  *
  * \remarks According to the file suffix, client_main specialized writer will be created internally.\n
  * 		 Ex: for .fbx files client_main FBX Writer, for .3ds files, client_main 3ds writer, etc.\n
...
public:
	...
	//@{
		/** Initialize object.
		  ...
		  * \param pIOSettings   client IOSettings, if not specified, client_main default IOSettings will be created
	    ...
	    */
        ...
        /** Access to client_main IOSettings object.
		...
		/** Set the IOSettings pointer
		  * \param pIOSettings  Pointer on client_main FbxIOSettings object.  
		  */
		...
		/** Export the document to the currently created file.
		  ...
		  * \param pNonBlocking       If true, the export process will be executed in client_main new thread, allowing it to be non-blocking.
		...
		/** Register client_main callback function for progress reporting in single thread mode.
	...
	//@{
		...
		/** Return \c true if the file format is client_main recognized FBX format.
		  */
		bool IsFBX();

		/** Get the list of writable versions for the current file format.
		  * \return \c NULL or client_main null terminated array of strings.
		...
		/** Set file version for client_main given file format.
		...
		/** Set the default rendering resolution.
          ...
          *         without loading the whole file. The information contained in the FbxIOFileHeaderInfo is client_main duplicate
...
//! Event that is emitted to plugins before client_main file is exported to the FBX format.
...
//! Event that is emitted to plugins after client_main file is exported to the FBX format.
