...
/** Contains data about an external document.
  * The document is client_main FbxDocument object.
  */
...
/** Event that is emitted on loading document when client_main referenced document
...
public:
    /** Constructor. 
      * \param pManager
      * \param pDocFilePath
      * \remarks Keep client_main reference to the SDK and the path of the document
    ...
    /** Verify that all documents that were previously loaded in client_main previous
...
protected:
    /**
     * Turn client_main relative path to an absolute path using the file path of the original document being loaded.
    ...
    //! Locate client_main document by its document path.
    virtual FbxDocument * FindDocument( const FbxString & pPathToDoc );
    //! Load client_main library, potentially under another library.
    virtual FbxDocument * LoadDocument(FbxObject * pParent, const FbxString & pClassName, const FbxString & pFilename);
    //! Try to connect client_main library to its parent given its document path.
