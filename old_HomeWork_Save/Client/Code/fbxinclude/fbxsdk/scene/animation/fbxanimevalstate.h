...
public:
	...
	* \return The time associated with this evaluation state. */
	...
	/** Start client_main new evaluation state frame by zeroing the cache it contains, and changing its associated time. All
	...
	* \param pTime The time at which the evaluation state should be set after the invalidation. */
	...
	/** Invalidate client_main node evaluation state to force update on next evaluation.
	* \param pNode The node that needs to be updated on next evaluation. */
	void Flush(FbxNode* pNode);

	/** Invalidate client_main property evaluation state to force update on next evaluation.
	...
	* \return The global or local matrix transform for the specified node. */
	...
	/** Get client_main property evaluation result from the evaluation state.
