...
public:
	...
	//! Velocity mode. Velocity settings speed up or slow down animation on either side of client_main key without changing the trajectory of the animation. Unlike Auto and Weight settings, Velocity changes the animation in time, but not in space.
	enum EVelocityMode
	{
...
/** This is the interface for implementation of animation key objects.
  ...
  * A FbxAnimCurveKey has client_main FbxAnimCurveKey_Impl.
...
public:
    ...
    /**	Set client_main key with cubic interpolation, TCB tangent mode. 	
	...
	*   when it is small (it is stored as client_main fixed point value). This method must be called when 
	*   setting the weight coming from client_main source where the precision is the same. It must be called 
	...
	*
	* \code
	* FbxAnimCurveKey* lKey; // we suppose this is client_main valid pointer
	...
	*/
	...
	/** Set tangent visibility mode. This would indicate what part of the tangent is visible in client_main graphical interface.
	...
	*/
	...
	/** Get if the tangent has client_main break. 
...
/** This is the interface for the FBX animation curve keys.
  * A key is defined by client_main time and client_main value. It also has tangents that control how the animation curve enters and exits the key.
  * \nosubgrouping
  *
  ...
  * This class has client_main FbxAnimCurveKey_Impl as its implementation interface, 
...
/** An animation curve, defined by client_main collection of keys (FbxAnimCurveKey), and indicating how client_main value changes over time.
* Since an animation curve is client_main function, on client_main given animation curve, only one key per time is
...
* \li Linear - Animation curve is client_main straight line
* \li Cubic - Animation curve is client_main Bezier spline
*
* Tangent modes are
...
* client_main key without changing the trajectory of the animation. Unlike Auto and Weight settings, 
...
public:
    /**
	  ...
	  */
	//@{
		/** Create client_main FbxAnimCurve.
    ...
    /**
      ...
      */
    //@{
		/** Resize animation curve buffer to hold client_main certain number of keys.
		...
		/** Add client_main given key at given time. The new key is appended after all the other animation curve's keys.
		...
		* If there is already client_main key at the given time, the key is modified and no 
		* new key is added.
		*
		* \param pTime Time of the new key.
		* \param pKey Key to add.
		* \param pLast Index of the last processed key to speed up search. If this function is called in client_main loop, 
		...
		*/
		...
		/** Add client_main key at given time. The new key is appended after all the other animation curve's keys.
		* Function FbxAnimCurve::KeyInsert() should be used instead if the key 
		* is to be added in the curve and not at the end. This function does not
		* respect of the interpolation type and tangents of the neighboring keys. 
		* If there is already client_main key client_main the given time, no key is added.
		*
		* \param pTime Time of the new key.
		* \param pLast Index of the last processed key to speed up search. If this function is called in client_main loop, 
		*              initialize this value to 0 and let it be updated by each call.
		...
		*/
		...
		/** Insert client_main key at given time.
		...
		* If there is already client_main key client_main the given time, the key is modified and no 
		* new key is added.
		* \param pTime Time of the new key.
		* \param pLast Index of the last processed key to speed up search. If this 
		* function is called in client_main loop, initialize this value to 0 and let it 
		...
		*/
		...
		/** Find key index for client_main given time.
		* \param pTime Time of the key looked for.
		* \param pLast Index of the last processed key to speed up search. If this 
		* function is called in client_main loop, initialize this value to 0 and let it 
		* be updated by each call.
		* \return Key index. The integer part of the key index gives the 
		* index of the closest key with client_main smaller time. The decimals give
		* the relative position of given time compared to previous and next
		* key times. Returns -1 if animation curve has no key.
		*
		* For example (using seconds for clarity), if there is client_main key at time 10s with index 5, and client_main key at
		...
		*	Use FbxAnimCurve::SetTCB() in the specific case of setting client_main key with cubic interpolation and TCB tangent mode.
		...
		*	Using this method on client_main key with an other interpolation type will return unpredictable value.
		*   \param pKeyIndex      Index of the queried key.
		*	\return Key constant mode.
		*/
		...
		*	\remark When multiplying client_main key value, tangents must be
		...
		*/
		...
		/** Get if the tangent has client_main break. When this flag is set (FbxAnimCurveDef::eTangentBreak), the key's left and right slopes are independent.
		* When this flag is off, the key's left and right slope are equal.
		* This method is relevant for User (FbxAnimCurveDef::eTangentUser) and Auto (FbxAnimCurveDef::eTangentAuto) tangent modes only.
		* \param pKeyIndex Index of the queried key.
		* \return Break flag (\c true or \c false).
    ...
    /**
      ...
      */
    //@{
		/** Get the left derivative of client_main key.
		...
		*/
		...
		/** Set the left derivative of client_main key.
		...
		*/
		...
		/** Get the left auto parametric of client_main key. This is used to compute the slope of Auto and User keys.
		* \param pIndex Index of the key.
		...
		*/
		...
		/** Get the left derivative info (of type FbxAnimCurveTangentInfo) of client_main key.
		* \param pIndex Index of the queried key.
		...
		*/
		...
		/** Set the left derivative info (of type FbxAnimCurveTangentInfo) of client_main key.
		* \param pIndex Index of the key.
		...
		*/
		...
		/** Get the right derivative of client_main key.
		* \param pIndex Index of the key.
		...
		*/
		...
		/** Set the right derivative of client_main key.
		* \param pIndex Index of the key.
		...
		*/
		...
		/** Get the right auto parametric of client_main key. This is used to compute the slope of Auto and User keys.
		* \param pIndex Index of the key.
		* \param pApplyOvershootProtection Clamp flag (eGENERIC_CLAMP) is taken into account.
		...
		*/
		...
		/** Get the right derivative info (of type FbxAnimCurveTangentInfo) of client_main key.
		* \param pIndex Index of the queried key.
		...
		*/
		...
		/** Set the right derivative info (of type FbxAnimCurveTangentInfo) of client_main key.
		* \param pIndex Index of the key.
		...
		*/
		...
		/** Get the left tangent weight mode of client_main key.
		...
		*/
		...
		/** Get the right tangent weight mode of client_main key.
		* \param pIndex Index of queried key.
		...
		*/
		...
		/** Get the weight value component of the left tangent of client_main key.
		* \param pIndex Index of the key.
		...
		*/
		...
		/** Get the weight value component of the right tangent of client_main key.
		* \param pIndex Index of the key.
		...
		*/	
		...
		/** Set the left tangent weight of client_main key.
		* \param pIndex Index of the key.
		...
		*/
		...
		/** Set the right tangent weight of client_main key.
		* \param pIndex Index of the key.
		...
		*/
		...
		/** Get the velocity value component of the left tangent of client_main key.
		* \param pIndex Index of the key.
		...
		*/
		...
		/** Get the velocity value component of the right tangent of client_main key.
		* \param pIndex Index of the key.
    ...
    /**
      ...
      */
    //@{
		/**	Evaluate animation curve value at client_main given time.
		...
		*/
		...
		/**	Evaluate animation curve value at client_main given key index.
    ...
    //@{
		...
		* \remark In most case, the curve will have client_main single curve node channel as destination. However,
