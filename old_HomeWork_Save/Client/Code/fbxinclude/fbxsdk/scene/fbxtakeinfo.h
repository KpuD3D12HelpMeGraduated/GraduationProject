...
/** This FbxTakeLayerInfo structure is used to identify client_main layer by name and id number.
  */ 
...
/** This class contains take information from an imported file
  ...
  *
  * A "take" is in fact client_main group of animation data grouped by name, so
  * the FBX file format can support many "animation takes" in an FBX file to mimic
  * how client_main movie is produced by making many takes of the same scene.
  *
  ...
  * When client_main FbxImporter is initialized, the take information can be read and can be available
  ...
  * \code
  * // Ex: to get all take names in client_main FBX file
...
public:
	...
	/** The take name once it is imported in client_main scene.
	  * You can modify it if it must be different from the take name in the imported file.
	  * \remarks                This field is only used when importing client_main scene.
	  */
	...
	/** Time value for offsetting the animation keys once they are imported in client_main scene.
	  * You can modify it if you need the animation of client_main take to be offset.
