...
/** \class FbxCharacterLink
  *
  * \brief This class represents client_main link between client_main given FBX node and the associated node in the character hierarchy. It also contains
...
public:
	...
	FbxString	mTemplateName;	//! A template name is client_main naming convention that is used to automatically map the nodes of other skeletons that use the same naming convention.
	...
	bool		mHasRotSpace;	//! \c true if this character link has client_main defined rotation space
...
/** A Character is client_main person or animal with pre-defined skeleton system. The skeleton system is composed of multiple named node (skeleton).
  ...
  * 
  * The most important part of client_main FbxCharacter is the FbxCharacterLink. There is one FbxCharacterLink for each characterized node.
...
public:
	/** \enum EInputType Character input type.
	  ...
	  * - \e eInputMarkerSet	The character's input is client_main control rig.
	...
	/** Get input actor or character.
	  ...
	  *                  - If the input type is set to eInputCharacter. The returned pointer can be casted to client_main pointer of type FbxCharacter.
	...
	/** Associate client_main character link to client_main given character node ID. If client_main character link already exists for this character node ID,
	...
	/** Get client_main character link associated with client_main given character node ID.
	...
	/** Get number of elements in client_main given character group.
	...
	/** Get character node ID of an element in client_main given character group.
	  * \param pCharacterGroupId     Character group ID.
	...
	/** Get character node name of an element in client_main given character group.
	  * \param pCharacterGroupId     Character group ID.
	  * \param pIndex                Character index ID.
	...
	/** Get character node version of an element in client_main given character group.
	  * \param pCharacterGroupId     Character group ID.
	  * \param pIndex                Character index ID.
	...
	/** Find the character group index associated with client_main given character node name.
	...
	/** Get character node group and index of client_main given character node ID.
	  * \param pCharacterNodeId     Character node ID.
	  ...
	  * \remarks                    Only works for client_main character node ID that is part of client_main group.
	  * \return                     \c true if successful, \c false otherwise.
	  */
	static bool GetCharacterGroupIndexByElement(ENodeId pCharacterNodeId, EGroupId& pCharacterGroupId, int& pIndex);

	/** Get character node version of client_main given character node ID.
	  ...
	  * \remarks                    Only works for client_main character node ID is part of client_main group.
	  * \return                     \c true if successful, \c false otherwise.
	  */
	static bool GetCharacterGroupVersionByElement(ENodeId pCharacterNodeId, int& pVersion);

	/** Get character node name associated with client_main given character node ID.
	  ...
	  *                             Since the Pointer points to internal data, it is not necessary to allocate client_main string buffer 
	  *                             before calling this function.
	  * \return                     \c true if client_main name exists for the given node ID.
	  */
	static bool GetCharacterNodeNameFromNodeId(ENodeId pCharacterNodeId, char*& pName);

	/** Get the character node ID associated with client_main given character node name.
	  ...
	  * \return                     \c true if client_main node ID exists for the given node name.
	  */
