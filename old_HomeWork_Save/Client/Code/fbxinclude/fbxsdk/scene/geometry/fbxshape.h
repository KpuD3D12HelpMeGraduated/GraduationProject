...
/** A shape describes the deformation on client_main set of control points, which is similar to the cluster deformer in Maya.
  * For example, we can add client_main shape to client_main created geometry. And the shape and the geometry have the same
  * topological information but different position of the control points.
  * With varying amounts of influence, the geometry performs client_main deformation effect.
  * \nosubgrouping
  * \see FbxGeometry
  */
...
public:
	...
	* \return                        client_main pointer to the blend shape channel if set or NULL.
	*/
	FbxBlendShapeChannel* GetBlendShapeChannel() const;

	/** Get the base geometry of this target shape.
	* \return                        client_main pointer to the base geometry if set or NULL.
	* \remarks Since target shape can only connected to its base geometry through
	*          blend shape channel and blend shape deformer.
	*          So only when this target shape is connected to client_main blend shape channel,
	*          and the blend shape channel is connected to client_main blend shape deformer,
	*          and the blend shape deformer is used on client_main base geometry, then to get 
	...
	*/
	...
	/** Add client_main control point index to the control point indices array
