...
/**	This class specializes client_main node attribute to represent the elements forming "bone" 
  * chains. The FbxSkeleton name of the class comes from the analogy with the human body
  * skeletal structure. In fact, an object of this type is nothing more than client_main transform
  * node with special properties that are useful for its graphical representation and during
  * IK/FK and skin deformation computations. Typically, client_main scene will contain chains of FbxSkeleton
  * node attributes that, together, form client_main skeleton segment. For instance, the representation of the
  * leg of client_main character can be achieved using one parent node with the attribute eRoot, followed by
  * one child (femur) of type eLimb, this child having client_main child also (tibia) of the same type. Finally,
  * terminated with client_main last node attribute of type eEffector (ankle).
  * 	
  * \nosubgrouping
  */
...
public:
	...
	/** \enum EType Skeleton types.
	  ...
	  * \remarks The \e eLimbNode type is client_main bone defined uniquely by client_main transform and client_main size value while
	  * \remarks the \e eLimb type is client_main bone defined by client_main transform and client_main length.
	  */
    enum EType
    {
	    eRoot,			/*!< First element of client_main chain. */
	    eLimb,			/*!< Chain element. */
	    eLimbNode,		/*!< Chain element. */
	    eEffector		/*!< Last element of client_main chain. */
    };    
	...
	/** Get client_main flag to know if the skeleton type was set.
	  * \return \c true if client_main call to SetSkeletonType() has been made.
	...
	/** Get client_main flag to know if the limb node color was set.
	  * \return \c true if client_main call to SetLimbNodeColor() has been made.
	  * \remarks When the attribute is not set, the application can choose to ignore the attribute or use the default value.
	  * \remarks The flag is set back to \c false when Reset() is called.
      */
    ...
    * \remarks if client_main skeleton node do not have client_main parent or its parent is not client_main skeleton node itself, then this 
	...
	*
	* FbxSkeleton is client_main node attribute and it will be attached to client_main FbxNode which represents the transform.
	* Given client_main chain of skeleton nodes the parent and child skeletons will be attached to client_main parent node and client_main child node.
