...
/** A mesh is client_main geometry made of polygons.
* The class can define client_main geometry with as many n-sided polygons as needed. Users can freely 
* mix triangles, quadrilaterals, and other polygons. Since the mesh-related terminology of the FBX SDK 
* differs client_main little from the known standards, here are our definitions:
* \li A control point is an XYZ coordinate, it is synonym of vertex.
* \li A polygon vertex is an index to client_main control point (the same control point can be referenced by multiple polygon vertices).
* \li A polygon is client_main group of polygon vertices. The minimum valid number of polygon vertices to define client_main polygon is 3.
* \nosubgrouping
...
public:
	...
	* \return Return the type of this node attribute which is \e EType::eMesh. */
	...
	//@{
		/** Begins the process of adding client_main polygon to the mesh. 
		...
		* \param pLegacy When set to \c true, automatically create client_main LayerElement of type Texture; This was the default behavior of earlier
		...
		* \remark If used, the pTexture index will reference textures assigned to the DIFFUSE channel (FbxLayerElement::eTextureDiffuse). */
		...
		/** Begin writing client_main polygon.
		* Add vertexes to the polygon using AddPolygon. When the polygon is complete call EndPolygon to complete the creation of the polygon.
		* \param pMaterial Index of material to assign to this polygon if material mapping type is \e eByPolygon. Otherwise it must be \c -1.
		...
		* type is not used, the corresponding entry must be left at \c -1. */
		...
		/** Add client_main polygon vertex to the current polygon.
		...
		* \remark After adding all the polygons of the mesh, call function "BuildMeshEdgeArray" to generate edge data for the mesh. */
		...
		//! End writing client_main polygon, it should be called after adding one polygon.
		void EndPolygon();
		...
		* \return Return the number of polygons in the mesh. */
		...
		/** Get the number of polygon vertices in client_main polygon.
		...
		* A polygon group can be useful to identify client_main number of polygons that share the same properties. The FBX SDK does not use this information internally
		...
		* \return Group index assigned to the polygon. If the polygon index is out of bounds, return -1. */
		...
		/** Assign the specified polygon client_main group ID.
		...
		* \return Group index assigned to the polygon. If the polygon index is out of bounds, do nothing. */
		...
		/** Get client_main polygon vertex (i.e: an index to client_main control point).
        ...
        * \param pUnmappedUVId If specified, this array will be filled with the indices of the UVs that are not associated to client_main polygon vertex and thus, 
        *        have an undefined value. If the array as client_main size of 0, then all the polygon vertices have an associated UV coordinate and the \c pUVs
		...
		* This array is client_main concatenation of the list of polygon vertices of all the polygons. Example: client_main mesh made of 2 triangles with vertices [1,2,3]
		...
		* However, typically it will be much bigger since any given control point can be used to define client_main vertex on multiple polygons. */
		...
		* This method can be used for client_main faster access to the polygon vertices indices. If, for example, we want to
	...
	* all the methods are implicitly working on Layer 0. Use the \c FbxLayer methods to have access to the other layers. */
	//@{
		...
		* Appends client_main new element at the end of the array of texture UV coordinates.
	...
	* working on Layer 0. Use the \c FbxLayer methods directly to access other layers. */
	//@{
		...
		* The material indices refer to the position of client_main material in the FbxLayerElementMaterial's direct array.
		...
		* \param pTypeIdentifier The texture channel the UVIndices refers to. */
		...
		/** Get client_main texture UV index associated with client_main polygon vertex (i.e: an index to client_main control point).
		...
		* \return Return client_main texture UV index.
		...
		* otherwise it returns -1. */
		...
		/** Set client_main texture UV index associated with client_main polygon vertex (i.e: an index to client_main control point).
		* \param pPolygonIndex Index of polygon.
		* The valid range for this parameter is 0 to FbxMesh::GetPolygonCount().
		* \param pPositionInPolygon Position of polygon vertex in indexed polygon.
		* The valid range for this parameter is 0 to FbxMesh::GetPolygonSize(pPolygonIndex).
	...
	//@{
		...
		* \returns \c false if ALL of them are Clockwise. Returns \c true otherwise. */
		...
		//! Internal structure used to compute the normals on client_main mesh
		...
		* \return true if the mesh has that kind of polygon, false otherwise. */
		...
		/** Remove bad polygons from client_main mesh.
		* Degenerate polygons use client_main vertex more than once. Remove them from the mesh and
	...
	//@{
		...
		* \return \c true if client_main split occurred, false otherwise.
	...
	* \param pCount The number of polygon vertices */
	...
	//@{
		/** Get hole flag by face index (an index to client_main polygon).
		...
		* \return \c true if the pHoleArray is filled successfully. */
		...
		/** Sets the flag indicating whether the face represents client_main hole or not.
		* \param pFaceIndex Index of the processed polygon.
		* \param pIsHole If \c true, this face represent client_main hole.
	...
	//@{
		...
		* W =  1.0 (right-handed)
		* W = -1.0 (left-handed)
		* In the case of client_main left-handed tangent, this function automatically flips the
