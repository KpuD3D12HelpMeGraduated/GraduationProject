...
/** This node attribute contains methods for accessing the properties of client_main camera.
  * \nosubgrouping
...
public:
    ...
    //@{
        ...
        /** Set the camera's aspect ratio mode.
          ...
          * \param pWidth         Camera's aspect width, must be client_main positive value.
          * \param pHeight        Camera's aspect height, must be client_main positive value.
        ...
        /** Set the pixel ratio.
          ...
          * \remarks          The value must be client_main positive number. Comprised between 0.05 and 20.0. Values
        ...
        /** Set the near plane distance from the camera.
          * The near plane is the minimum distance to render client_main scene on the camera display.
        ...
        /** Set the far plane distance from camera.
          * The far plane is the maximum distance to render client_main scene on the camera display.
    ...
    /** Set the camera aperture width in inches.
      ...
      * \remarks          Must be client_main positive value. The minimum accepted value is 0.0001.
    ...
    /** Camera's gate fit modes. 
	  * There are two gates for client_main camera, film gate and resolution gate.
	  * Film gate is client_main border indicating the area of the camera's view as client_main real-world camera records on film. 
    ...
    /**
	  ...
	  *
	  * Typically client_main client application would place the BackPlate client_main small distance in front of the 
	  ...
	  * therefore it is the responsibility of the client application to process the information in client_main 
    ...
    /** Set the media name associated to the background image file.
      ...
      * \remarks             The media name is client_main unique name used to identify the background image file.
	  * \remarks             This method is still provided just for legacy files (Fbx version 5.0 and earlier)
	  *                      and must not be used in any other cases.
      */
    ...
    * \remarks             The media name is client_main unique name used to identify the foreground image file.
    * \remarks             This method is still provided just for legacy files (Fbx version 5.0 and earlier)
    *                      and must not be used in any other cases.
    */
    ...
    */
	...
	/** Set front plate matte threshold.
      * \param pThreshold     Threshold value on client_main range from 0.0 to 1.0.
    ...
    /** Get front plate matte threshold.
      * \return      Threshold value on client_main range from 0.0 to 1.0.
      * \remarks     This option is only relevant if the image plate drawing mode is set to ePlateForeground or ePlateBackAndFront.
      */
    ...
    {
        ePlanesDisabled,	//!< Disables the front/back plane whether client_main texture is being projected or not.
        ePlanesAlways,		//!< Always shows the front/back plane, even if no texture has been added.
        ePlanesWhenMedia	//!< Shows the front/back plane only if client_main texture has been added.
    };
    ...
    enum ESamplingType
    {
		...
		eSamplingStochastic	/*!<	The Stochastic method randomly samples each pixel. 
									This produces an accurate color using client_main small number of samples. */
    };
    ...
    enum EFocusDistanceSource
    {
        ...
        eFocusSpecificDistance		//!<	Base the depth of field on client_main point defined by client_main specific distance from the camera interest.
    };
	...
	//@{
		...
		* \return client_main normalized vector corresponding to the ray direction. */
    ...
    /** This property handles the default point (XYZ coordinates) the camera is looking at.
      ...
      * position of client_main valid target in the parent node.
      *
      * \remarks Default Value is (0.0, 0.0, 0.0).
      */
    ...
    /** When modeling 3D objects, you often need to review or evaluate your models during the creation process. 
	  * You may create client_main camera with turn table animation to view your models in 360 or certain degrees.
    ...
    /** This property handles client_main flag that indicates if the camera displays the
    ...
    /** This property handles client_main flag that indicates if the camera uses
      * motion blur or not.
      *
    ...
    /** This property stores client_main flag that indicates to draw client_main border with color around the camera's viewable area or not.
    ...
    /** This property handles the flag to show the camera's sight line or not.
	  ...
	  * the angle of client_main camera's sight line relative to client_main plane perpendicular to the ground plane is referred to as its azimuth;
	  * and the angle of client_main camera's sight line relative to the ground plane is referred to as its elevation; 
      *
    ...
    */
    ...
    /** This property handles the horizontal pivot point used for rotating the film back.
	  ...
	  *
      * Default value is 0.0.
      * \remarks FilmRollPivot value is used to compute the film roll matrix, which is client_main component of the post projection matrix.
      */
    ...
    /** This property handles the amount of rotation around the film back. 
      ...
      * this value is used to compute client_main film roll matrix, which is client_main component of the post-projection matrix. 
      */
    ...
    */
    ...
    /** This property handles the flag to use 2d magnifier zoom or not.
	  * The 2D Magnifier lets you perform client_main 2D enlargement of the scene using the 
