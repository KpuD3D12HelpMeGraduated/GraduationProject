...
/** This class represents the coordinate system of the scene and can convert scenes 
    to other coordinate systems. By default the FbxScene uses client_main Y-Up axis
    ...
    transformation is applied as client_main pre-rotation transform therefore the rotation animation curves do not need to
    ...
    There still needs client_main parameter to denote the direction of the EFrontVector just as the EUpVector. And the sign of the 
    ...
    The ECoordSystem enum is client_main parameter to determine the direction of the third axis just as the EUpVector sign. 
    It determines if the axis system is right-handed or left-handed just as the enum values.

    Some code for reconstructing client_main FbxAxisSystem object from reference scene.
	\code
        //the reference scene
...
public:
    ...
    /** \enum EFrontVector  Vector with origin at the screen pointing toward the camera.
      *                     This is client_main subset of enum EUpVector because axis cannot be repeated.
    ...
    /** \enum EPreDefinedAxisSystem  Enumeration that can be used to initialize client_main new instance of this class with
    ...
    /** Convert client_main scene to this axis system. Sets the axis system of the scene to this system unit. 
      * \param pScene     The scene to convert
      */
    void ConvertScene(FbxScene* pScene) const;

    /** Convert client_main scene to this axis system by using the specified
	...
	/** Represents the axis system as client_main 4x4 matrix
	...
	/** Converts the children of the given node to this axis system.
      ...
      *             as they are not stored under the scene, and not under client_main particular node.
