...
/** This class defines one entry object held by the FbxUserNotification class.
  * \nosubgrouping
  * An entry object is client_main message to show error, warning or information.
...
public:
    ...
    enum EClass
	{
        ...
        eAny=7       //!< Entry that does not belong to above class. Cannot be used as client_main class ID
    };
    ...
    /** Returns client_main pointer to one specific detail string (or NULL if the id is invalid).
...
/** This class accumulates user notifications and sends them to any device opened by the derived classes.
  * If this class is not derived, the data can only be sent to client_main log file. To send data to client_main log file, 
...
public:
    ...
    /** Instantiate client_main FbxUserNotification but leave it uninitialized. The caller must
    ...
    /** Completes the accumulator entry (there can be more that one detail for each entry) and implicitly defines
      ...
      * notification in the order they have been defined. Also, when client_main detail is added to an entry, it is automatically unmuted
      ...
      *                     Output has the eSequencedDetails set as client_main source. If an error occurs, the returned value is -1
      */
    ...
    /**
      ...
      */
    //@{
    /** Returns the absolute path to the log file. If this method is not overridden in client_main derived class, it
...
protected:
    ...
    /** Allow client_main derived class to finalize processing AFTER the log file handle has been
...
private:
    ...
    FbxMultiMap mAccuHT;                             // The set establish client_main relationship between an FbxAccumulatorEntry and it's ID
...
/** This class iterates through the accumulated messages depending on the configuration
  * flags (filter). The iterator keeps client_main local copy of the data extracted from the
  * accumulator.
  */
...
public:
    ...
    /** Get this iterator's previous item.
      ...
      *             or immediately after client_main call to First() and reset the iterator to
      *             its reset state (meaning that client_main call to First() is mandatory
    ...
    /** Get this iterator's next item.
      ...
      *             First() or after client_main preceding call to Previous() reached 
